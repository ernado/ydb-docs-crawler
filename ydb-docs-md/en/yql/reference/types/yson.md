---
title: "Yson"
url: "https://ydb.tech/docs/en/yql/reference/types/yson?version=v26.1"
doc_path: "en/yql/reference/types/yson"
version: "v26.1"
lang: "en"
...
2. [Attributes](yson.md#attributes): an arbitrary dictionary that can be attached to a literal of any type (including scalars).
...
1. [Strings](yson.md#string) (`string`);
2. [Signed](yson.md#int) and [unsigned](yson.md#uint) 64-bit integers (`int64` and `uint64`);
3. [Double-precision floating-point](yson.md#double) numbers (`double`);
4. [Boolean](yson.md#boolean) (`boolean`);
5. The [special entity type](yson.md#entity) with a single literal (`#`).

Scalar types usually have both text and binary representations.

There are two **composite** types:

1. [List](yson.md#list) (`list`);
2. [Map](yson.md#map) (`map`).
...
Entity is an atomic scalar value with no payload of its own. It is useful in many scenarios—for example, `entity` often represents `null`. An `entity` may still carry [attributes](yson.md#attributes).
...
Not all of these are used in YSON; some appear in [YPath](yson.md#ypath).
...
| ```<br><a=b;>c<br>{a=b;}<br>1;2;3;<br>``` | ```<br><a=b>c<br>{a=b}<br>1;2;3<br>``` |
...
| --- | --- |
| column0 | column1 |
| ```json<br>{<br>  "0-25-3ec012f-406daf5c": {<br>    "a": {<br>      "$attributes": {<br>        "why": "I can just do it"<br>      },<br>      "$value": 1<br>    },<br>    "b": 2<br>  }<br>}<br>``` | `"I can just do it"` |
...
| ```json<br>{<br>    "a": "z",<br>    "x": "y"<br>}<br>``` | ```json<br>{<br>    "it_is_string": true<br>}<br>``` | ```json<br>true<br>``` | ```json<br>{<br>    "abc": 123,<br>    "def": 456<br>}<br>``` | ```json<br>{<br>    "abc": 234,<br>    "entity0123": null,<br>    "xyz": 789<br>}<br>``` | ```json<br>{<br>    "$attributes": {<br>        "here_you_can_store": "something"<br>    },<br>    "$value": null<br>}<br>``` | ```json<br>null<br>``` | ```json<br>{<br>    "$attributes": {<br>        "a": "z",<br>        "x": "y"<br>    },<br>    "$value": [<br>        {<br>            "abc": 123,<br>            "def": 456<br>        },<br>        {<br>            "abc": 234,<br>            "entity0123": null,<br>            "xyz": 789<br>        }<br>    ]<br>}<br>``` | ```json<br>{<br>    "a": {<br>        "$attributes": {<br>            "a": "z",<br>            "x": "y"<br>        },<br>        "$value": [<br>            {<br>                "abc": 123,<br>                "def": 456<br>            },<br>            {<br>                "abc": 234,<br>                "xyz": 789<br>            }<br>        ]<br>    },<br>    "b": {<br>        "38 parrots": [<br>            38<br>        ],<br>        "str": {<br>            "$attributes": {<br>                "it_is_string": true<br>            },<br>            "$value": "hello"<br>        }<br>    }<br>}<br>``` |
