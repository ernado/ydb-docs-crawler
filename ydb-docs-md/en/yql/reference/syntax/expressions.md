---
title: "Expressions"
url: "https://ydb.tech/docs/en/yql/reference/syntax/expressions?version=v26.1"
doc_path: "en/yql/reference/syntax/expressions"
version: "v26.1"
lang: "en"
source_path: "en/core/yql/reference/syntax/expressions.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/en/core/yql/reference/syntax/expressions.md"
description: "String concatenation. Executed using the binary operator ||. As with other binary operators, if the data on either side is NULL, the result is also NULL."
...
| 3 | `a\|\|b` | [String concatenation](expressions.md#concatenation) | Left |
...
Matching an empty value (`NULL`). Since `NULL` is a special value [equal to nothing](../types/optional.md#null_expr), the ordinary [comparison operators](expressions.md#comparison-operators) can't be used to match it.
...
Comparing of two values. Unlike the regular [comparison operators](expressions.md#comparison-operators), NULLs are treated as equal to each other.  
...
> [!WARNING]
> Unlike a similar keyword in Python, in YQL `IN` **DOES NOT** search for a substring inside a string. To search for a substring, use the function [String::Contains](../udf/list/string.md) or [LIKE/REGEXP](expressions.md#check-match) mentioned above.
...
- To specify the target type in the case of explicit type casting, see [CAST](expressions.md#cast).
...
Each named expression has a scope. It starts immediately after the definition of a named expression and ends at the end of the nearest enclosed namescope (for example, at the end of the query or at the end of the body of the [lambda function](expressions.md#lambda), [ACTION](action.md#define-action)).  
...
Anonymous argument names are also supported for [lambda functions](expressions.md#lambda), [ACTION](action.md#define-action).
...
- [Named subqueries](expressions.md#named-nodes): `$foo = SELECT * FROM T;` (in this case, `$foo` is also a table expression)
...
- Table context: after [FROM](select/from.md). In this case, table expressions work as expected: for example, `$input = SELECT a, b, c FROM T; SELECT * FROM $input` returns a table with three columns. The table context also occurs after [UNION ALL](select/index.md#unionall), [JOIN](select/join.md#join);
- Vector context: after [IN](expressions.md#in). In this context, the table expression must contain exactly one column (the name of this column doesn't affect the expression result in any way). A table expression in a vector context is typed as a list (the type of the list element is the same as the column type in this case). Example: `SELECT * FROM T WHERE key IN (SELECT k FROM T1)`;
