---
title: "Migration to Configuration V1"
url: "https://ydb.tech/docs/en/devops/configuration-management/migration/migration-to-v1?version=v26.1"
doc_path: "en/devops/configuration-management/migration/migration-to-v1"
version: "v26.1"
lang: "en"
...
This document contains instructions for migrating from [configuration V2](../configuration-v2/config-overview.md) to [configuration V1](../configuration-v1/index.md).

> [!NOTE]
> This guide is intended for emergency situations when unexpected problems arise after [migrating to configuration V2](migration-to-v2.md) and a rollback to configuration V1 is required, for example, for subsequent rollback to a YDB version below v25-1. This procedure is not required in normal operation mode.

## Initial State

Migration to configuration V1 is only possible if the cluster uses [configuration V2](../configuration-v2/config-overview.md). This can be achieved:
...
You can verify the successful completion of the migration by checking the configuration version on the cluster nodes using one of the methods described in the article [Checking Configuration Version](../check-config-version.md). All cluster nodes should use configuration `v1`.
