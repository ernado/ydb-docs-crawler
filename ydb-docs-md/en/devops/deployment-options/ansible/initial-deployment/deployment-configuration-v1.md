...
url: "https://ydb.tech/docs/en/devops/deployment-options/ansible/initial-deployment/deployment-configuration-v1?version=v26.1"
doc_path: "en/devops/deployment-options/ansible/initial-deployment/deployment-configuration-v1"
version: "v26.1"
lang: "en"
...
3. **Filesystem paths to block devices.** In the `all.children.ydb.vars.ydb_disks` section, the template assumes `/dev/vda` is for the operating system and the following disks, such as `/dev/vdb`, are for the YDB storage layer. Disk labels are created by the playbooks automatically, and their names can be arbitrary.
4. **System version.** In the `ydb_version` parameter, specify the YDB version to install. The list of available versions is on the [downloads](../../../../downloads/ydb-open-source-database.md) page.
...
To speed up and simplify the initial YDB deployment, the configuration file already contains most of the cluster setup. You only need to replace the placeholder host FQDNs with the actual ones in the `hosts` and `blob_storage_config` sections.
