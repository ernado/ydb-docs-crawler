---
title: "en/devops/deployment-options/manual/bridge-management"
url: "https://ydb.tech/docs/en/devops/deployment-options/manual/bridge-management?version=v26.1"
doc_path: "en/devops/deployment-options/manual/bridge-management"
version: "v26.1"
lang: "en"
source_path: "en/core/devops/deployment-options/manual/bridge-management.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/en/core/devops/deployment-options/manual/bridge-management.md"
description: "Managing a cluster in bridge mode. Feature of Yandex Enterprise Database."
revision: "e9f541853a7760e5c0d0babc071d86df7f523cf5"
---

# en/devops/deployment-options/manual/bridge-management
...
Below are typical operations for a cluster in [bridge mode](../../../concepts/bridge.md) using the [corresponding YDB CLI commands](../../../reference/ydb-cli/commands/bridge/index.md).
...
If disconnecting the current `PRIMARY` and it was not possible to [change it in advance](bridge-management.md#switchover), these operations can be combined by specifying the new `PRIMARY` in the `--new-primary` argument, which must be in the `SYNCHRONIZED` state.
...
> [!WARNING]
> Before starting planned maintenance, always verify using the [list](bridge-management.md#list) command that the pile disconnection operation has completed successfully and all pile are in the expected state.
...
Immediately after the operation starts, the pile transitions to the `NOT_SYNCHRONIZED` state and a background data synchronization process starts; when synchronization completes, the pile automatically becomes `SYNCHRONIZED`. After waiting for this state, you can [switch the `PRIMARY` role to this pile](bridge-management.md#switchover) if needed.
