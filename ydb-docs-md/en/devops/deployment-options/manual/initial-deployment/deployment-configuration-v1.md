---
title: "Deploying a Cluster Using Configuration V1"
url: "https://ydb.tech/docs/en/devops/deployment-options/manual/initial-deployment/deployment-configuration-v1?version=v26.1"
doc_path: "en/devops/deployment-options/manual/initial-deployment/deployment-configuration-v1"
version: "v26.1"
lang: "en"
...
Before deploying the system, complete the preparatory steps. Review the [Deployment Preparation](deployment-preparation.md) document.

## Prepare Configuration Files {#config}

Prepare the YDB configuration file according to your chosen topology (see [cluster topology](deployment-preparation.md)). Examples for each supported topology are provided below in tabs — select and use the one that fits your case.
...
To speed up and simplify the initial YDB deployment, the configuration file already contains most cluster setup settings. Replace the default host FQDNs with your actual ones in the `hosts` and `blob_storage_config` sections.
...
For more detailed information about creating the configuration file, see [YDB Cluster Configuration](../../../../reference/configuration/index.md).

## Copy TLS Keys and Certificates to Each Server {#tls-copy-cert}

Copy the prepared TLS keys and certificates to a protected directory on each YDB cluster node. Below are sample commands to create a protected directory and copy the key and certificate files.
...
- Manually
  ...
  Run the YDB data storage service on each static cluster node:
...
After starting the static nodes, verify they are running via the YDB built-in web interface (Embedded UI):
...
![Manual installation, static nodes running](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/devops/deployment-options/_assets/manual_installation_1.png)

## Initialize the Cluster {#initialize-cluster}

The cluster initialization operation configures the set of static nodes listed in the cluster configuration file for storing YDB data.
...
1. Install the YDB CLI as described in the [documentation](../../../../reference/ydb-cli/install.md).

2. Set the password for the `root` account using the token obtained earlier:
   ...
   Replace `passw0rd` with the desired password. Save the password in a separate file. Subsequent commands as the `root` user will use the password passed with the `--password-file <path_to_user_password>` option. You can also save the password in a connection profile, as described in the [YDB CLI documentation](../../../../reference/ydb-cli/profile/index.md).
...
In the command examples above, `<node.ydb.tech>` is the FQDN of the server running any dynamic node serving the `/Root/testdb` database. When connecting via SSH to a YDB dynamic node, you can use `grpcs://$(hostname -f):2136` to get the FQDN.

When running account creation and group assignment commands, the YDB CLI client will prompt for the `root` user password. To avoid repeated password entry, create a connection profile as described in the [YDB CLI documentation](../../../../reference/ydb-cli/profile/index.md).

## Test the Created Database {#try-first-db}

1. Install the YDB CLI as described in the [documentation](../../../../reference/ydb-cli/install.md).
2. Create a test row-oriented table (`test_row_table`) or column-oriented table (`test_column_table`):
...
To check access to the YDB built-in web interface, open `https://<node.ydb.tech>:8765` in your browser, where `<node.ydb.tech>` is the FQDN of the server running any static YDB node.

Configure your browser to trust the Certificate Authority that issued certificates for the YDB cluster. Otherwise, you will see a warning about an untrusted certificate.

If authentication is enabled in the cluster, the browser will prompt for login and password. After entering valid credentials, the built-in web interface welcome page will appear. The available features and user interface are described in [Using the embedded web UI](../../../../reference/embedded-ui/index.md).

> [!NOTE]
> A common way to provide access to the YDB built-in web interface is to set up a fault-tolerant HTTP balancer using `haproxy`, `nginx`, or similar software. HTTP balancer configuration details are beyond the scope of the standard YDB installation guide.

## Installing YDB in Unprotected Mode

> [!WARNING]
> We do not recommend using the unprotected YDB mode for production or application development.

The installation procedure above assumes YDB deployment in the standard protected mode.

The unprotected YDB mode is intended for test scenarios, primarily related to YDB software development and testing. In unprotected mode:
...
To install YDB for operation in unprotected mode, follow the procedure above with the following exceptions:
...
2. Remove the `security_config`, `interconnect_config`, and `grpc_config` sections from the cluster node configuration files.
3. Use simplified commands to run static and dynamic cluster nodes: omit options specifying certificate and key file names; use the `grpc` protocol instead of `grpcs` when specifying connection endpoints.
4. Skip the authentication token step before cluster initialization and database creation, as it is not needed in unprotected mode.
5. The cluster initialization command has the following format:
...
7. When accessing the database from the YDB CLI and applications, use grpc instead of grpcs and do not use authentication.
