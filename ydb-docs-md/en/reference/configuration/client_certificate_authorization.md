...
url: "https://ydb.tech/docs/en/reference/configuration/client_certificate_authorization?version=v26.1"
doc_path: "en/reference/configuration/client_certificate_authorization"
version: "v26.1"
lang: "en"
...
| Key | Description |
| --- | --- |
| `request_client_certificate` | Request a valid client certificate for node connections.  <br>Allowed values:<br>- `false` — A certificate is not required (used by default if the parameter is omitted).<br>- `true` — A certificate is required for all node connections. |
...
| `require_same_issuer` | Require that the value of the "Issuer" field (typically containing the Certification Authority name) is the same for both client (database node) and server (storage node) certificates.   <br>Allowed values:<br>- `true` — The values must be the same (used by default if the parameter is omitted).<br>- `false` — The values can be different (allowing client and server certificates to be issued by different Certification Authorities). |
