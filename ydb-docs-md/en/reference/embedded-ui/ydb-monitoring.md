---
title: "YDB Monitoring"
url: "https://ydb.tech/docs/en/reference/embedded-ui/ydb-monitoring?version=v26.1"
doc_path: "en/reference/embedded-ui/ydb-monitoring"
version: "v26.1"
lang: "en"
...
YDB Monitoring is a set of web pages that display the system health according to a range of different aspects. The pages contain lists of components and their current parameters. Many components have a health [color indicator](ydb-monitoring.md#colored_indicator) before the name.
...
![Monitoring_main_page](https://ydb.tech/docs/docs-assets/ydb-platform--ydb/rev/7580679a5c9e32c15be9989745f34e270cb4e4f1/en/reference/embedded-ui/_assets/monitoring_main_page.png)
...
Next, you will find lists of [tenants](ydb-monitoring.md#tenant_list_page) and [nodes](ydb-monitoring.md#node_list_page) on the Tenants and Nodes tabs, respectively.
...
If you click on the tenant's path, you can go to the [tenant page](ydb-monitoring.md#tenant_page).
...
To open the [node page](ydb-monitoring.md#node_page), click the host name.
...
The node page has the Storage and Tablets tabs with a [list of storage groups](ydb-monitoring.md#node_storage_page) and a [list of tablets](ydb-monitoring.md#node_tablets_page), respectively.
...
Under the indicator, you can see a list of tablets, where each tablet is shown as a small [color indicator](ydb-monitoring.md#colored_indicator) icon. When you hover over the indicator, the tablet summary is shown:
...
- **Pools**: Total CPU utilization by the tenant nodes broken down by internal stream pools (for more information about pools, see the [tenant page](ydb-monitoring.md#tenant_page)).
...
- **Storage**: [List of storage groups](ydb-monitoring.md#tenant_storage_page) showing the nodes and block store volumes hosting each VDisk.
- **Compute**: [List of nodes](ydb-monitoring.md#tenant_compute_page) showing the nodes and tablets running on them.
- **Schema**: [Tenant schema](ydb-monitoring.md#tenant_scheme) that enables you to view tables, execute YQL queries, view a list of the slowest queries and the most loaded shards.
- **Network**: [Cluster network health](ydb-monitoring.md#tenant_network).

### List of storage groups in the tenant {#tenant_storage_page}

Similarly to the [list of groups in the node](ydb-monitoring.md#node_storage_page), this page shows a list of storage groups in the given tenant. Storage groups are grouped by storage pools in the list. Each pool can be expanded into a list of groups.
...
On the left side of the page, you can see the tenant's nodes as squares of [health indicators](ydb-monitoring.md#colored_indicator).
