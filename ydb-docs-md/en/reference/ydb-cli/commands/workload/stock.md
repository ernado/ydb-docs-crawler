---
title: "Stock load"
url: "https://ydb.tech/docs/en/reference/ydb-cli/commands/workload/stock?version=v26.1"
doc_path: "en/reference/ydb-cli/commands/workload/stock"
version: "v26.1"
lang: "en"
source_path: "en/core/reference/ydb-cli/commands/workload/stock.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/en/core/reference/ydb-cli/commands/workload/stock.md"
description: "Simulates a warehouse of an online store: creates multi-product orders, gets a list of orders per customer. Types of load. This load test runs 5 types of load:"
...
- [user-hist](stock.md#get-customer-history): Reads the specified number of orders made by the customer with id = 10000. This creates a workload to read the same rows from different threads.
- [rand-user-hist](stock.md#get-random-customer-history): Reads the specified number of orders made by a randomly selected customer. A load that reads data from different threads is created.
- [add-rand-order](stock.md#insert-random-order): Generates an order at random. For example, a customer has created an order of 2 products, but hasn't yet paid for it, hence the quantities in stock aren't decreased for the products. The database writes the data about the order and products. The read/write load is created (the INSERT checks for an existing entry before inserting the data).
- [put-rand-order](stock.md#submit-random-order): Generates an order at random and processes it. For example, a customer has created and paid an order of 2 products. The data about the order and products is written to the database, product availability is checked and quantities in stock are decreased. A mixed data load is created.
- [put-same-order](stock.md#submit-same-order): Creates orders with the same set of products. For example, all customers buy the same set of products (a newly released phone and a charger). This creates a workload of competing updates of the same rows in the table.
...
- `init options`: [Initialization options](stock.md#init_options).
...
- `workload type`: The [types of workload](stock.md#workload_types).
- `global workload options`: The [global options for all types of load](stock.md#global_workload_options).
...
- `global workload options`: The [global options for all types of load](stock.md#global_workload_options).
- `specific workload options`: [Options of a specific load type](stock.md#customer_history_options).
...
- `global workload options`: The [global options for all types of load](stock.md#global_workload_options).
- `specific workload options`: [Options of a specific load type](stock.md#random_customer_history_options).
...
- `global workload options`: The [global options for all types of load](stock.md#global_workload_options).
- `specific workload options`: [Options of a specific load type](stock.md#insert_random_order_options).
...
- `global workload options`: The [global options for all types of load](stock.md#global_workload_options).
- `specific workload options`: [Options of a specific load type](stock.md#submit_random_order_options).
...
- `global workload options`: The [global options for all types of load](stock.md#global_workload_options).
- `specific workload options`: [Options of a specific load type](stock.md#submit_same_order_options).
