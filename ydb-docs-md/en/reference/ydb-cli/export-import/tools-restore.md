---
title: "Importing data from the file system"
url: "https://ydb.tech/docs/en/reference/ydb-cli/export-import/tools-restore?version=v26.1"
doc_path: "en/reference/ydb-cli/export-import/tools-restore"
version: "v26.1"
lang: "en"
...
The `admin cluster restore` command restores a cluster from a backup on the file system. The backup must have been previously exported or prepared manually as described in the [File structure of an export](file-structure.md) article:
...
When restoring a cluster' metadata, databases and their administrators are created. Refer to [Database](tools-restore.md#db) for further details on restoring databases.
...
The `admin database restore` command restores the database from a backup on the file system. The backup must have been previously exported with the `admin database dump` command or prepared manually as described in the [File structure of an export](file-structure.md) article:

> [!WARNING]
> Currently, this command does not restore column table data (such data is not included in the backup). To import data into column tables, you can use [external data sources](../../../concepts/datamodel/external_data_source.md). For more information, see [Reading Data from an External Table Pointing to S3 (Object Storage)](../../../concepts/query_execution/federated_query/s3/external_table.md).
...
### Required parameters {#mandatory1}

`-i <PATH>` or `--input <PATH>`: Path to the directory in the client system from which the data will be imported.

### Optional parameters {#optional1}
...
The `tools restore` command creates the items of the database schema in the database, and populates them with the data previously exported there with the `tools dump` command or prepared manually as per the rules from the [File structure of an export](file-structure.md) article:
...
### Required parameters {#mandatory2}
