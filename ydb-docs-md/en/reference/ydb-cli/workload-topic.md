---
title: "Topic load"
url: "https://ydb.tech/docs/en/reference/ydb-cli/workload-topic?version=v26.1"
doc_path: "en/reference/ydb-cli/workload-topic"
version: "v26.1"
lang: "en"
...
Applies load to your YDB [topics](../../concepts/datamodel/topic.md), using them as message queues. You can use a variety of input parameters to simulate production load: message number, message size, target write rate, and number of consumers and producers.
...
1. [Initialize the load](workload-topic.md#init).

2. Run one of the available load types:

   - [write](workload-topic.md#run-write): Generate messages and write them to the topic asynchronously.
   - [read](workload-topic.md#run-read): Read messages from the topic asynchronously.
   - [full](workload-topic.md#run-full): Read and write messages asynchronously in parallel.
...
This type of load reads messages from the topic asynchronously. To make sure that the topic includes messages, run the [write load](workload-topic.md#run-write) before you start reading.
