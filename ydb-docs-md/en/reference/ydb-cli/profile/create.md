...
url: "https://ydb.tech/docs/en/reference/ydb-cli/profile/create?version=v26.1"
doc_path: "en/reference/ydb-cli/profile/create"
version: "v26.1"
lang: "en"
...
You can set connection parameter values for the profile being created or updated through the [command line](create.md#cmdline) or request them [in interactive mode](create.md#interactive) in the console.
...
- `<connection options>` are [connection parameters](../connect.md#command-line-pars) to be written to the profile. You need to specify at least one connection parameter; otherwise the command will run [in interactive mode](create.md#interactive).
...
Creating/replacing a `local` profile to connect to a local YDB database deployed using [quick start](../../../quickstart.md):
...
2. Enter the [endpoint](../../../concepts/connect.md#endpoint) or don't save this parameter for the profile:
...
3. Enter the [database name](../../../concepts/connect.md#database) or don't save this parameter for the profile:
...
4. Select the authentication mode or don't save this parameter for the profile:
   ...
   All the available authentication methods are described in [Authentication](../../../security/authentication.md). The set of methods and text of the hints may differ from those given in this example.
...
5. In the last step, you'll be prompted to activate the created profile to be used by default. Choose 'n' (No) until you read the article about [Activating a profile and using the activated profile](activate.md):
