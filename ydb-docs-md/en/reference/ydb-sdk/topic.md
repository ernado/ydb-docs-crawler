...
url: "https://ydb.tech/docs/en/reference/ydb-sdk/topic?version=v26.1"
doc_path: "en/reference/ydb-sdk/topic"
version: "v26.1"
lang: "en"
source_path: "en/core/reference/ydb-sdk/topic.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/en/core/reference/ydb-sdk/topic.md"
description: "This article provides examples of how to use the YDB SDK to work with topics. Before performing the examples, create a topic and add a consumer."
revision: "e9f541853a7760e5c0d0babc071d86df7f523cf5"
---

# Working with topics

This article provides examples of how to use the YDB SDK to work with [topics](../../concepts/datamodel/topic.md).
...
- C++
  ...
  Example of adding an [important consumer](../../concepts/topic.md#important-consumer) and setting two days [retention time](../../concepts/topic.md#retention-time) for the topic:
...
Only connections with matching [producer and message group](../../concepts/topic.md#producer-id) identifiers are currently supported (`producer_id` shoud be equal to `message_group_id`). This restriction will be removed in the future.
...
For more details on using data compression for topics, see [here](../../concepts/topic.md#message-codec).

{% list tabs %}

- C++

  The message compression can be set on the [write session creation](topic.md#start-writer) with `Codec` and `CompressionLevel` settings. By default, GZIP codec is chosen.
...
- C++ If no ProducerId is specified on write session setup, the session runs in no-deduplication mode. The example below demonstrates such a session setup:
  ...
  If, on other hand, you want to ensure deduplication is enabled, you can specify the ProducerId option or call the `DeduplicationEnabled()` method from WriteSessionSettings. The '[Connecting to a topic for message writes](topic.md#start-writer)' section has an example of write session that has deduplication enabled.

- Go In **ydb-go-sdk**, when you create a writer without explicitly passing `topicoptions.WithWriterProducerID`, the SDK still assigns a producer ID (it generates one automatically). A mode equivalent to omitting `ProducerId` in the C++ example above is not available in the current SDK version.

- Java This functionality is not currently supported.
...
- Java
  ...
  >
  > - It should be an active transaction (that has an id) from one of YDB services. I.e., [Table](https://github.com/ydb-platform/ydb-java-sdk/blob/master/table/src/main/java/tech/ydb/table/transaction/TableTransaction.java) or [Query](https://github.com/ydb-platform/ydb-java-sdk/blob/master/query/src/main/java/tech/ydb/query/QueryTransaction.java).
...
Reading messages from a topic can be done by specifying a Consumer associated with that topic, as well as without a Consumer. If a Consumer is not specified, the client application must calculate the offset for reading messages on its own. A more detailed example of reading without a Consumer is discussed in the [relevant section](topic.md#no-consumer).

A Consumer can be created on [creating](topic.md#create-topic) or [altering](topic.md#alter-topic) a topic.  
...
The server stores the [consumer offset](../../concepts/datamodel/topic.md#consumer-offset). After reading a message, the client should [send a commit to the server](topic.md#commit). The consumer offset changes and only uncommitted messages will be read in case of a new connection.

You can read messages without a [commit](topic.md#no-commit) as well. In this case, all uncommited messages, including those processed, will be read if there is a new connection.

Information about which messages have already been processed can be [saved on the client side](topic.md#client-commit) by sending the starting consumer offset to the server when creating a new connection. This does not change the consumer offset on the server.

Data from topics can be read in the context of [transactions](topic.md#read-tx). In this case, the reading offset will only advance when the transaction is committed. On reconnect, all uncommitted messages will be read again.
...
- C++
  ...
  The starting position of a specific partition read can be set during `TStartPartitionSessionEvent` handling.
   ...
   Additionally, there is a `commitOffset` parameter that tells the server to consider all messages with lesser offsets [committed](topic.md#commit).
...
- Java
  ...
  - Asynchronous API
    ...
    Override `onStopPartitionSession(StopPartitionSessionEvent event)` on your `ReadEventHandler` (see [Connecting for reads](topic.md#start-reader)).
