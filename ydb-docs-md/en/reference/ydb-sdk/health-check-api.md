...
url: "https://ydb.tech/docs/en/reference/ydb-sdk/health-check-api?version=v26.1"
doc_path: "en/reference/ydb-sdk/health-check-api"
version: "v26.1"
lang: "en"
...
`SelfCheck` method provides information in the form of a [set of issues](health-check-api.md#example-emergency) which could look like this:
...
| --- | --- | --- |
| `ReturnVerboseStatus` | `bool` | If `ReturnVerboseStatus` is specified, the response will also include a summary of the overall health of the database in the `database_status` field ([Example](health-check-api.md#example-verbose)). Default is false. |
...
For the full response structure, see the [ydb_monitoring.proto](https://github.com/ydb-platform/ydb/blob/main/ydb/public/api/protos/ydb_monitoring.proto) file in the YDB Git repository.  
...
The shortest `HealthCheck` response looks like [this](health-check-api.md#examples) . It is returned if there is nothing wrong with the database.
...
| Field | Description |
| --- | --- |
| `self_check_result` | enum field which contains the [database check result](health-check-api.md#selfcheck-result) |
...
| `issue_log.status` | enum field which contains the [issue status](health-check-api.md#issue-status) |
...
| `issue_log.type` | Issue category (by subsystem). Each type is at a certain level and interconnected with others through a [rigid hierarchy](health-check-api.md#issues-hierarchy) (as shown in the picture above). |
| `issue_log.level` | Issue [nesting depth](health-check-api.md#issues-hierarchy). |
| `database_status` | If the settings include `ReturnVerboseStatus` parameter, the `database_status` field will be populated.   <br>This field offers a comprehensive summary of the overall health of the database.   <br>It is designed to provide a quick overview of the database's condition, helping to assess its health and identify any major issues at a high level. [Example](health-check-api.md#example-verbose). For the full response structure, see the [ydb_monitoring.proto](https://github.com/ydb-platform/ydb/blob/main/ydb/public/api/protos/ydb_monitoring.proto) file in the YDB Git repository. |
...
![cards_hierarchy](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/reference/ydb-sdk/_assets/hc_cards_hierarchy.png)
...
![issues_hierarchy](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/reference/ydb-sdk/_assets/hc_types_hierarchy.png)
...
#### Tablets/Followers are dead {#tablets/followers-are-dead}
...
The number of cores is calculated by dividing the quota by the period (quota/period)(quota / period)(quota/period).
