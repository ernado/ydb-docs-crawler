---
title: "Using the merge process to implement SCD2 in YDB"
url: "https://ydb.tech/docs/en/analyst/practical-guides/scd/scd2-merge?version=v26.1"
doc_path: "en/analyst/practical-guides/scd/scd2-merge"
version: "v26.1"
lang: "en"
...
This article describes the implementation of the [Slowly Changing Dimensions Type 2 (SCD2)](index.md#scd2) pattern in YDB using the change merge process.
...
2. The target table `dimension_scd2_final` to store the resulting data.
3. An external application should periodically run a query to merge changes accumulated in the `dimension_scd_changes` table into the `dimension_scd2_final` table.
4. To deliver data from staging tables into SCD2 format, it is convenient to use the built-in YDB [transfer mechanism](../../../concepts/transfer.md).
...
## Creating a table to receive all Changes `dimension_scd_changes` {#creating-a-table-to-receive-all-changes}
...
## Creating the final SCD2 table `dimension_scd2_final` {#creating-the-final-scd2-table}
...
A special query is used to transform data from the changes table into SCD2 format and load it into the final table. This query should be run regularly—with whatever frequency you want to update data in the final table. For automatic execution, you can use the YDB [integration](../../../integrations/orchestration/airflow.md) with Apache Airflow™:
