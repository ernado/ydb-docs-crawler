---
title: "Managing YDB using Terraform"
url: "https://ydb.tech/docs/en/dev/terraform?version=v26.1"
doc_path: "en/dev/terraform"
version: "v26.1"
lang: "en"
...
[Terraform](https://www.terraform.io/) can create, delete, and modify the following objects inside a YDB cluster:
...
1. Deploy the [YDB](../devops/index.md) cluster
2. Create a database (described in paragraph 1 for the appropriate type of cluster deployment)
3. Install [Terraform](https://developer.hashicorp.com/terraform/install)
4. Install and configure [Terraform provider for YDB](https://github.com/ydb-platform/terraform-provider-ydb/)

## Configuring the Terraform provider to work with YDB {#setup}
...
## Using the Terraform provider YDB {#work-with-tf}
...
If you are using the creation of `ydb_table_changefeed` or `ydb_topic` resources and authorization is not enabled on the YDB server, then in the DB config [config.yaml](../reference/configuration/index.md) you need to specify:
...
- `connection_string` — (required) [connection string](terraform.md#connection_string).
- `column` — (required) column properties (see the [column](terraform.md#column) argument).
- `family` - (optional) is a column group (see the [family](terraform.md#family) argument).
...
- `ttl` — (optional) TTL (see the [ttl](terraform.md#ttl) argument).
- `partitioning_settings` — (optional) partitioning settings (see the argument [partitioning_settings](terraform.md#partitioning-settings)).
...
- `type` — (required) [YQL data type](../yql/reference/types/primitive.md) columns. Simple column types are allowed. However, container types cannot be used as data types of table columns.
- `family` - (optional) is the name of the column group (see the [family](terraform.md#family) argument).
...
- `connection_string` — [connection string](terraform.md#connection_string). Specified if `table_id` is not specified.
...
- `mode` - (required) is the mode of operation of the [change data capture](../yql/reference/syntax/alter_table/index.md#changefeed-options).
- `format` - (required) is the format of the [change data capture](../yql/reference/syntax/alter_table/index.md#changefeed-options).
...
- `consumer` - (optional) is a reader of the change data capture (see the argument [#consumer](terraform.md#consumer)).
...
### Usage examples {#usage-examples-{manage-examples}}
