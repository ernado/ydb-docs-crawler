...
url: "https://ydb.tech/docs/en/dev/vector-indexes?version=v26.1"
doc_path: "en/dev/vector-indexes"
version: "v26.1"
lang: "en"
source_path: "en/core/dev/vector-indexes.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/en/core/dev/vector-indexes.md"
description: "Vector indexes are specialized data structures that enable efficient vector search in multidimensional spaces. Unlike secondary indexes, which optimize searchin"
...
Vector indexes in YDB address the nearest neighbor search problem using [similarity or distance functions](../yql/reference/udf/list/knn.md#functions-distance). Distance/similarity function parameters for vector indices:
...
Internally, a vector index consists of hidden index tables named `indexImpl*Table`. In [selection queries](vector-indexes.md#select) using the vector index, the index tables will appear in [query statistics](query-execution-optimization/query-plans-optimization.md). For more on the structure of the vector index, see the dedicated article [Vector index type: vector_kmeans_tree](vector-indexes-kmeans-tree-type.md).
...
### Vector Index Type `vector_kmeans_tree` {#kmeans-tree-type1}
...
### Overlapping Clusters {#overlap-clusters1}
...
1. Create a [covering index](vector-indexes.md#covering) so that only index tables are involved in search queries.
