---
title: "Vector index type: vector_kmeans_tree"
url: "https://ydb.tech/docs/en/dev/vector-indexes-kmeans-tree-type?version=v26.1"
doc_path: "en/dev/vector-indexes-kmeans-tree-type"
version: "v26.1"
lang: "en"
source_path: "en/core/dev/vector-indexes-kmeans-tree-type.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/en/core/dev/vector-indexes-kmeans-tree-type.md"
description: "Vector indexes are specialized data structures that enable efficient vector search in multidimensional spaces. They are described in the overview; this article"
...
![First-level clusters](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/_assets/vector_kmeans_tree_1.png)
...
![Second-level clusters](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/_assets/vector_kmeans_tree_2.png)
...
![Cluster centroids](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/_assets/vector_kmeans_tree_4.png)

After choosing a cluster at one level, YDB moves to the next level and considers the clusters that belong to the cluster chosen at the previous level. YDB compares distances between cluster centroids and the query vector and selects the cluster with the smallest distance. Approximate search assumes that if a cluster’s centroid is closest to the query, the vectors in that cluster are also closest to the query.

![Choosing a first-level cluster](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/_assets/vector_kmeans_tree_5.png)

Each first-level cluster is split into second-level clusters, and so on. The algorithm “descends” the tree step by step, each time selecting a smaller cluster until it reaches a leaf cluster that holds vectors instead of centroids. YDB scans these vectors and returns the requested number of nearest neighbors. By limiting the number of clusters at each level, YDB keeps search complexity at `O(1)`.

![Choosing a second-level cluster](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/_assets/vector_kmeans_tree_6.png)

If a leaf cluster has fewer vectors than requested, YDB returns all vectors found. To improve recall in such cases, use the `KMeansTreeSearchTopSize` parameter described below.

## Index parameters {#index-settings}

YDB lets you configure the number of levels in the vector index with the `levels` parameter and the number of clusters per level with the `clusters` parameter. Choose these values based on the expected number of vectors and cluster characteristics: network latency between nodes, network throughput, and CPU performance.

Clusters in the vector index tree are stored in distributed storage. On each step down the tree, YDB reads from distributed storage and receives binary data with the list of vectors over the network. The time for this operation depends on datacenter hardware, network load, and the amount of data transferred between clusters.
...
If a cluster’s centroid is closest to the query, the vectors in that cluster are usually closer to the query than vectors in other clusters. For complex data topologies this may not hold, so YDB allows scanning vectors in more than one cluster per level.

The number of clusters participating in search is set by the `KMeansTreeSearchTopSize` parameter. By default it is 1: YDB picks one nearest cluster at the first level, then requests its child clusters at the second level over the network, picks one nearest again, and so on. This parameter is not set at index creation time but [for the search](../yql/reference/syntax/select/vector_index.md#kmeanstreesearchtopsize).
...
However, after finding the leaf cluster, YDB must run a `SELECT` against the table to fetch the vectors (the exact number depends on the size of the leaf cluster).
...
- If you filter after search, you may get fewer results than needed and recall drops. You would have to repeat the search with a higher limit, which is also slow. In YDB, filtering is integrated into the vector index structure and algorithm, so rows that do not match the filter are excluded from consideration immediately.

## Index internals {#index-structure}

The index is implemented with two extra tables: the level table (`indexImplLevelTable`) and the posting table (`indexImplPostingTable`). The level table stores centroids for the cluster tree. YDB queries this table when moving from level to level during vector search. For example, if the vector index is built over 27 vectors with `levels=2` and `clusters=3`, the level table looks like this:

![Level table](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/_assets/vector_kmeans_tree_7.png)

For each centroid in the level table, the parent cluster at the level above is also stored. YDB uses this to request only the centroids that belong to clusters selected at the previous level when moving down.
...
![Posting table](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/_assets/vector_kmeans_tree_8.png)
...
![Filtered index architecture](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/en/core/_assets/vector_kmeans_tree_9.png)

### Structure of table `indexImplLevelTable` {#structure-of-table}
...
### Structure of table `indexImplPostingTable` {#structure-of-table1}
...
### Example structure of `indexImplPostingTable` with a covering index {#with-a-covering-index}
...
### Example structure of `indexImplPrefixTable` for a filtered index {#for-a-filtered-index}
