---
title: "Setting up the transaction execution mode"
url: "https://ydb.tech/docs/en/recipes/ydb-sdk/tx-control?version=v26.1"
doc_path: "en/recipes/ydb-sdk/tx-control"
version: "v26.1"
lang: "en"
source_path: "en/core/recipes/ydb-sdk/tx-control.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/en/core/recipes/ydb-sdk/tx-control.md"
description: "To run your queries, first you need to specify the transaction execution mode in the YDB SDK."
revision: "e9f541853a7760e5c0d0babc071d86df7f523cf5"
---

# Setting up the transaction execution mode

To run your queries, first you need to specify the [transaction execution mode](../../concepts/transactions.md#modes) in the YDB SDK.

Below are code examples showing the YDB SDK built-in tools to create an object for the *transaction execution mode*.

## ImplicitTx

[ImplicitTx](../../concepts/transactions.md#implicit) mode allows executing a single query without explicit transaction control. The query is executed in its own implicit transaction that automatically commits if successful.
...
- Java
  ...
  - JDBC
    ...
    For **standalone** JDBC calls, the **current** driver implementation uses **Serializable Read/Write** — including from Spring Boot, ORMs, and other JDBC wrappers. **Snapshot** for reads is enabled via **`setReadOnly(true)`** (see [ImplicitTx](tx-control.md#implicittx)). Auto-commit semantics and explicit transactions are described there as well.
...
- Java
  ...
  - JDBC
    ...
    Online Read-Only from the Query API is **not** configured separately for **standalone** JDBC calls. For **snapshot** reads, call **`Connection.setReadOnly(true)`** (in Spring — `@Transactional(readOnly = true)`). For writes, see [ImplicitTx](tx-control.md#implicittx).
...
- Java
  ...
  - JDBC
    ...
    Stale Read-Only from the Query API is **not** configured separately for **standalone** JDBC calls. For **snapshot** reads, call **`Connection.setReadOnly(true)`** (in Spring — `@Transactional(readOnly = true)`). For writes, see [ImplicitTx](tx-control.md#implicittx).
...
- Java
  ...
  - JDBC
    ...
    For **standalone** **read-only** JDBC calls, **snapshot** mode is enabled when **`Connection.setReadOnly(true)`** is set on the connection (in Spring, **`@Transactional(readOnly = true)`** propagates this to the driver when the transaction starts). See [ImplicitTx](tx-control.md#implicittx) for details.
...
- Java
  ...
  - JDBC
    ...
    Snapshot Read-Write from the Query API is **not** set separately for **standalone** JDBC calls; for **writes**, the **current** driver implementation uses **Serializable Read/Write** (see [ImplicitTx](tx-control.md#implicittx) and [Serializable](tx-control.md#serializable)).
