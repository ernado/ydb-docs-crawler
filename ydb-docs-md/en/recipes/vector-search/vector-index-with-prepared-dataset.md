---
title: "Vector index with a prepared external dataset"
url: "https://ydb.tech/docs/en/recipes/vector-search/vector-index-with-prepared-dataset?version=v26.1"
doc_path: "en/recipes/vector-search/vector-index-with-prepared-dataset"
version: "v26.1"
lang: "en"
...
This article walks you through working with vector indexes in YDB. It uses an English Wikipedia text dataset (485,859 rows) prepared by the [Hugging Face](https://huggingface.co) community.

The article covers:

- [creating a table for loading data](vector-index-with-prepared-dataset.md#step1);
- [downloading the dataset](vector-index-with-prepared-dataset.md#step2);
- [importing data into the table](vector-index-with-prepared-dataset.md#step3);
- [building the vector index](vector-index-with-prepared-dataset.md#step4);
- [running vector search without the index](vector-index-with-prepared-dataset.md#step5);
- [running vector search with the index](vector-index-with-prepared-dataset.md#step6).

## Prerequisites

To run the examples in this article you need:

- A YDB database with [vector index](../../dev/vector-indexes.md) support enabled. For a simple single-node cluster setup, see [here](../../quickstart.md). For production deployment options, see [here](../../devops/deployment-options/index.md).

- On your machine:

  - The [YDB CLI](../../reference/ydb-cli/index.md);
  - Python 3;
...
Use the [YDB CLI](../../reference/ydb-cli/index.md) to import the file `wikipedia_embeddings_train.csv` into the table `wikipedia` created in [Step 1](vector-index-with-prepared-dataset.md#step1).

In the command below:

- `<endpoint>` — [endpoint](../../concepts/connect.md#endpoint) of your YDB database
- `<database>` — [database path](../../concepts/connect.md#database) of your YDB database

```bash
ydb -e <endpoint> -d <database> -v import file csv --path wikipedia --header wikipedia_embeddings_train.csv --timeout 30
```

In this example the test YDB server runs without authentication, so no user credentials are passed to the CLI. For CLI authentication, see [here](../../reference/ydb-cli/connect.md).

The file `wikipedia_embeddings_train.csv` is about 6.26 GB, so the upload may take a while. When it finishes, you can verify that all rows were loaded into the `wikipedia` table from [Step 1](vector-index-with-prepared-dataset.md#step1) with:
...
For YDB vector indexes, vectors must be in string format (see [data types](../../yql/reference/udf/list/knn.md#data-types)). The embeddings in the imported dataset (stored in the `emb` column) can be converted into the `embedding` column in the required format by running this script on your machine:
...
For general information on vector indexes, creation parameters, and current limitations, see [Vector Indexes](../../dev/vector-indexes.md).
...
To search for the 3 nearest neighbors of a given vector using the index `idx_vector` created in [Step 4](vector-index-with-prepared-dataset.md#step4), run:
