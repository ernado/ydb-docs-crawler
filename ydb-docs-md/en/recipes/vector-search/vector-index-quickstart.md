---
title: "Vector Index — Quick Start"
url: "https://ydb.tech/docs/en/recipes/vector-search/vector-index-quickstart?version=v26.1"
doc_path: "en/recipes/vector-search/vector-index-quickstart"
version: "v26.1"
lang: "en"
source_path: "en/core/recipes/vector-search/vector-index-quickstart.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/en/core/recipes/vector-search/vector-index-quickstart.md"
description: "This article will help you quickly get started with vector indexes in YDB using a simple model example."
...
- [creating a table with vectors](vector-index-quickstart.md#step1);
- [populating the table with data](vector-index-quickstart.md#step2);
- [building a vector index](vector-index-quickstart.md#step3);
- [performing vector search without an index](vector-index-quickstart.md#step4);
- [performing vector search with an index](vector-index-quickstart.md#step5).
...
For a description of the `Knn::ToBinaryStringFloat` function, see [KNN](../../yql/reference/udf/list/knn.md).
...
This command creates an index of the `vector_kmeans_tree` type. For more information about indexes of this type, see [Vector Index Type `vector_kmeans_tree`](../../dev/vector-indexes.md#kmeans-tree-type). In this model example, the parameter `clusters=2` is specified (splitting the set of vectors when building the index into two clusters at each level); for real data, values in the range from 64 to 512 are recommended.

For general information about vector indexes, their creation parameters, and current limitations, see the section [Vector Indexes](../../dev/vector-indexes.md).
...
First, the target vector is encoded into a binary representation using [`Knn::ToBinaryStringFloat`](../../yql/reference/udf/list/knn.md#functions-convert).
...
For detailed information about exact vector search without using vector indexes, see [KNN](../../yql/reference/udf/list/knn.md).

## Step 5. Searching the table using a vector index {#step5}

To search for the 3 nearest neighbors of the vector `[1.f, 1.f, 1.f, 1.f, 4.f]` using the `EmbeddingIndex` index that was created in [step 3](vector-index-quickstart.md#step3), execute the following query:
...
For more information about vector indexes, see [Vector Indexes](../../dev/vector-indexes.md).
