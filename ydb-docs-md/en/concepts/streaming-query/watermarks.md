---
title: "Watermarks"
url: "https://ydb.tech/docs/en/concepts/streaming-query/watermarks?version=v26.1"
doc_path: "en/concepts/streaming-query/watermarks"
version: "v26.1"
lang: "en"
source_path: "en/core/concepts/streaming-query/watermarks.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/en/core/concepts/streaming-query/watermarks.md"
description: "A watermark in stream processing ( stream processing ) is a monotonically increasing lower bound on the event times that may still arrive in the stream. When th"
...
YDB implements the watermark mechanism in [streaming queries](streaming-query.md): they are used to correctly close time-based aggregation windows ([HoppingWindow](../../yql/reference/syntax/select/group-by.md#group-by-hopping_window)) and ensure that the window result is emitted only when the system has confirmed the completeness of input data for that period.
...
Events that arrive after the watermark has passed the corresponding time range are considered late and are discarded. For more details on handling late events and configuring watermarks, see [Watermarks](../../dev/streaming-query/watermarks.md).
