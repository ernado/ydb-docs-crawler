---
title: "YDB Provider for LinqToDB"
url: "https://ydb.tech/docs/en/integrations/orm/linq2db?version=v26.1"
doc_path: "en/integrations/orm/linq2db"
version: "v26.1"
lang: "en"
source_path: "en/core/integrations/orm/linq2db.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/en/core/integrations/orm/linq2db.md"
description: "Overview. This guide explains how to use LinqToDB with YDB."
...
This guide explains how to use [LinqToDB](https://linq2db.github.io/) with YDB.

linq2db is a lightweight and fast ORM for .NET that provides type-safe [LINQ](https://learn.microsoft.com/en-us/dotnet/csharp/linq/) queries and precise SQL control. The YDB provider generates correct YQL, supports YDB types, schema generation, and bulk operations (Bulk Copy).

## Installing the YDB provider {#install-provider}
...
### Type Mapping between .NET and YDB {#types}
...
| `decimal` | `Decimal` | `Decimal(p,s)` | Default: Decimal(22,9). To use custom precision/scale, specify Decimal(p,s). [Example](linq2db.md#custom-precision-scale-example) |
| `float` | `Single` | `Float` | — |
...
| `DateOnly` / `DateTime` | `Date` | `Date32` | Wider date range. Use `DbType = "Date32"`. [Example](linq2db.md#dbtype-override-example) |
| `DateTime` | `DateTime` | `Datetime64` | Second precision, wider range. `DbType = "Datetime64"`. [Example](linq2db.md#dbtype-override-example) |
| `DateTime` / `DateTimeOffset` | `DateTime2` | `Timestamp64` | Microseconds, wider range. `DbType = "Timestamp64"`. [Example](linq2db.md#dbtype-override-example) |
| `TimeSpan` | `Interval` | `Interval64` | Wider interval range. `DbType = "Interval64"`. [Example](linq2db.md#dbtype-override-example) |
...
#### Option A — attribute (name + columns) {#option-a-—-attribute-name-columns}
...
#### Option B — extended parameters (ASYNC, COVER) via separate DDL {#option-b-—-extended-parameters-async,-cover-via-separate-ddl}
...
### Bulk operations: insert, update and delete {#bulk-operations-insert,-update-and-delete}
