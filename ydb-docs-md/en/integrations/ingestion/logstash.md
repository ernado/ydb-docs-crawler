---
title: "Integrating Logstash and YDB"
url: "https://ydb.tech/docs/en/integrations/ingestion/logstash?version=v26.1"
doc_path: "en/integrations/ingestion/logstash"
version: "v26.1"
lang: "en"
...
[Logstash](https://www.elastic.co/logstash) dynamically ingests, transforms, and ships data regardless of format or complexity. A Logstash pipeline can contain different types of plugins: input, output, and filter. The YDB Logstash plugins repository is hosted on [GitHub](https://github.com/ydb-platform/ydb-logstash-plugins) and contains the following plugins:

- **Storage Plugin** for persisting Logstash events in [row-oriented](../../concepts/datamodel/table.md#row-oriented-table) or [column-oriented](../../concepts/datamodel/table.md#olap-data-types) YDB tables;
- **Input Topic Plugin** for reading Logstash events from YDB [topics](../../concepts/datamodel/topic.md);
- **Output Topic Plugin** for sending Logstash events to YDB [topics](../../concepts/datamodel/topic.md).
...
This plugin allows storing the Logstash events stream in YDB tables for further analysis. This is especially useful with [column-oriented tables](../../concepts/datamodel/table.md#column-tables) optimized for handling Online Analytical Processing (OLAP) requests. Every [field](https://www.elastic.co/guide/en/logstash/current/event-dependent-configuration.html) of a Logstash event will be stored in a column with a corresponding name. Fields that do not match any column will be ignored.

### Configuration

The plugin configuration is done by adding a `ydb_storage` block in the `output` section of the Logstash pipeline [config file](https://www.elastic.co/guide/en/logstash/current/configuration.html). The plugin supports the standard set of [connection parameters](logstash.md#configure-ydb-connection) and a few additional options:
...
> [!WARNING]
> The Storage plugin doesn't check if the Logstash event has correct and unique values for the table's primary key. It uses the [bulk upsert method](../../dev/batch-upload.md) for storing data in YDB, and if multiple events have the same primary keys, they will overwrite each other. The primary key is recommended to contain those event fields that will be present in every event and can uniquely identify each event. If no such set of fields exists, add an extra column to the primary key and fill it with a random value using the `uuid_column` parameter.
...
To check that all sent messages are written to the table, run the query with [ydb sql](../../reference/ydb-cli/sql.md) (the `ydb table query execute -t scan` command is deprecated):
...
This plugin allows reading messages from YDB [topics](../../concepts/datamodel/topic.md) and transforming them into `Logstash` events.

### Configuration {#configuration1}

The plugin configuration is done by adding a `ydb_topic` block to the `input` section of the Logstash pipeline [config](https://www.elastic.co/guide/en/logstash/current/configuration.html). The plugin supports the standard set of [connection parameters](logstash.md#configure-ydb-connection) and a few additional options:
...
#### Setup plugin in Logstash pipeline config {#setup-plugin-in-logstash-pipeline-config1}
...
This plugin allows writing `Logstash` events to a YDB [topic](../../concepts/datamodel/topic.md).

### Configuration {#configuration2}

The plugin configuration is done by adding a `ydb_topic` block to the `output` section of the Logstash pipeline [config](https://www.elastic.co/guide/en/logstash/current/configuration.html). The plugin supports the standard set of [connection parameters](logstash.md#configure-ydb-connection) and a few additional options:

- `topic_path` — the required the full path of the topic for writing.

### Usage example {#usage-example2}

#### Create a topic {#create-a-topic1}
...
#### Setup plugin in Logstash pipeline config {#setup-plugin-in-logstash-pipeline-config2}
