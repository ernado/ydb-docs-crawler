---
title: "Connecting to YDB using the VS Code plugin"
url: "https://ydb.tech/docs/en/integrations/gui/vscode-plugin?version=v26.1"
doc_path: "en/integrations/gui/vscode-plugin"
version: "v26.1"
lang: "en"
...
[YDB for VS Code](https://github.com/ydb-platform/ydb-vscode-plugin) is a VS Code extension with native support for YDB. The plugin provides a specialized interface for working with YDB objects: a hierarchical navigator for tables, topics, views, external data sources, support for all authentication methods, a [YQL](../../concepts/glossary.md#yql) editor with syntax highlighting, visualization of execution plans, session and cluster monitoring, access control management (ACL), a built-in [MCP server](https://modelcontextprotocol.io/) for AI assistants, and other features.
...
- Cluster dashboard based on [YDB Embedded UI](../../reference/embedded-ui/index.md): CPU load, memory usage, network traffic (updates every 10 seconds).
...
- Built-in [MCP server](vscode-plugin.md#mcp) — direct access to databases from AI assistants (Claude Code and others).
- [Semantic search in YQL documentation](vscode-plugin.md#rag) (RAG) for AI-assisted query writing.
...
3. The connection creation form will open. Fill in the fields:
   ...
   | **Host** | Endpoint of the [endpoint](../../concepts/connect.md#endpoint) of the YDB cluster | `ydb.example.com` |
   | **Port** | Port (default `2135`) | `2135` |
   | **Database** | Path to the [database](../../concepts/glossary.md#database) | `/Root/database` |
   | **Monitoring URL** | URL of [YDB Embedded UI](../../reference/embedded-ui/index.md), used for the dashboard (filled in automatically based on the host, can be overridden) | `http://ydb.example.com:8765` |
   | **Secure connection (grpcs)** | Use a secure connection (`grpcs://`) | ☑ |
   | **Use RAG** | Enable [search in YQL documentation](vscode-plugin.md#rag) for this connection | ☑ |
...
5. Select the authentication method from the **Auth type** drop-down list (see [Authentication methods](vscode-plugin.md#auth-methods)).
...
The plugin supports all [authentication](../../security/authentication.md) methods available in YDB. The method is selected in the **Auth type** dropdown on the connection creation form.
...
Authentication by login and password. Specify the username in the **Username** field and the password in the **Password** field. Used if [login and password authentication](../../security/authentication.md#static-credentials) is enabled on the YDB server.
...
The plugin runs a built-in [MCP server](https://modelcontextprotocol.io/) (Model Context Protocol), which allows AI assistants (Claude Code and others) to directly execute queries against YDB databases configured in the plugin.
...
1. Make sure the YDB extension is running in VS Code and at least one connection is added in the **Connections** panel (see [Creating a connection](vscode-plugin.md#connection)).
...
| `ydb_yql_help` | `query`, `connection?` | Search YQL documentation (requires [RAG](vscode-plugin.md#rag) enabled) |
...
- **Semantic search (vector)** — requires a locally running [Ollama](https://ollama.com) with the [`nomic-embed-text`](https://ollama.com/library/nomic-embed-text) embedding model. See [Installing Ollama and the embedding model](vscode-plugin.md#install-ollama).
...
If the plugin is installed from the [Marketplace](vscode-plugin.md#install-marketplace), VS Code updates it automatically.
...
If the plugin is installed from a [VSIX file](vscode-plugin.md#install-vsix), automatic updates are not available — VS Code does not track updates for manually installed extensions.
