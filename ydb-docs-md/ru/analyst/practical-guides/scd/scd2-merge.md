---
title: "Использование процесса подмерживания изменения для реализации SCD2 в YDB"
url: "https://ydb.tech/docs/ru/analyst/practical-guides/scd/scd2-merge?version=v26.1"
doc_path: "ru/analyst/practical-guides/scd/scd2-merge"
version: "v26.1"
lang: "ru"
...
В этой статье описывается реализация паттерна [Slowly Changing Dimensions Type 2 (SCD2)](index.md#scd2) в YDB с использованием процесса подмерживания изменений.
...
2. Таблица-приёмник `dimension_scd2_final` для хранения результирующих данных.
3. Периодически внешнее приложение должно вызывать запрос, который будет подмерживать изменения данных, накопившиеся в таблице `dimension_scd_changes`, в таблицу `dimension_scd2_final`.
4. Для поставки данных из строковых таблиц для хранения их в формате SCD2 удобно использовать встроенный в YDB механизм [трансфера](../../../concepts/transfer.md).
...
## Создание таблицы для приёма всех изменений `dimension_scd_changes` {#sozdanie-tablicy-dlya-priyoma-vseh-izmenenij}
...
## Создание финальной SCD2 таблицы `dimension_scd2_final` {#sozdanie-finalnoj-scd2-tablicy}
...
## Загрузка данных в таблицу изменений {#zagruzka-dannyh-v-tablicu-izmenenij}
...
## Запрос для размещения изменений в формате SCD2 {#zapros-dlya-razmesheniya-izmenenij-v-formate-scd2}

Чтобы преобразовать данные из таблицы изменений в формат SCD2 и загрузить их в финальную таблицу, используется специальный запрос. Этот запрос нужно запускать регулярно — с такой периодичностью, с какой вы хотите обновлять данные в финальной таблице. Для автоматического запуска можно воспользоваться [интеграцию](../../../integrations/orchestration/airflow.md) YDB с Apache Airflow™:
...
## Получение данных из SCD2-таблицы {#poluchenie-dannyh-iz-scd2-tablicy}

### Получение актуальных данных {#poluchenie-aktualnyh-dannyh}
...
### Получение данных на определённый момент времени {#poluchenie-dannyh-na-opredelyonnyj-moment-vremeni}
...
### Получение истории изменений для конкретной записи {#poluchenie-istorii-izmenenij-dlya-konkretnoj-zapisi}
