---
title: "Список изменений YDB Server"
url: "https://ydb.tech/docs/ru/changelog-server?version=v26.1"
doc_path: "ru/changelog-server"
version: "v26.1"
lang: "ru"
...
description: "Версия 26.1 Версия 26.1.1.20. Дата выхода: 02 июля 2026. Функциональность."
...
- Доступны YQL-запросы [`SHOW CREATE TABLE`](yql/reference/syntax/show_create.md) и [`SHOW CREATE VIEW`](yql/reference/syntax/show_create.md) для получения DDL-выражений, необходимых для воссоздания структуры таблицы или представления.
- В `ALTER TABLE` добавлена поддержка значений по умолчанию при [`ADD COLUMN`](yql/reference/syntax/alter_table/columns.md) (`DEFAULT`).
- [Shuffle Elimination](concepts/query_execution/optimizer.md) включён в продакшене: оптимизатор может устранять лишние перераспределения данных при соединениях.
- Реализовано [резервное копирование и восстановление](reference/ydb-cli/export-import/file-structure.md) схемных объектов: [асинхронных репликаций](concepts/async-replication.md), [внешних источников данных](concepts/datamodel/external_data_source.md), [внешних таблиц](concepts/datamodel/external_table.md) и [трансферов](concepts/transfer.md).
- Кластер сохраняет работоспособность при недоступности [CMS](concepts/glossary.md#cms).
- Добавлена возможность [регистрации динамических узлов](devops/configuration-management/configuration-v1/node-authorization.md) по клиентским TLS-сертификатам.
- Для [LDAP-аутентификации](security/authentication.md) сервисного аккаунта поддерживается протокол SASL с механизмом EXTERNAL — см. [`enable_sasl_external_bind`](reference/configuration/auth_config.md#ldap-auth-config).
- В [асинхронной репликации](concepts/async-replication.md) поддержано зеркалирование [автопартиционированных топиков](concepts/datamodel/topic.md#autopartitioning); см. также [партиции топиков в CDC](concepts/cdc.md#topic-partitions).
- Расширена [диагностика TLI](reference/configuration/tli_config.md) (Transaction Lock Invalidation): конфигурация `tli_config`, [логирование](troubleshooting/performance/queries/tli-logging.md) и [системные представления](dev/system-views.md#top-tli-partitions).
- При [автопартиционировании по нагрузке](concepts/datamodel/table.md#auto_partitioning_by_load) учитывается CPU-нагрузка на лидера партиции и на все её реплики.
- [Потоковые запросы](dev/streaming-query/index.md) поддерживают [запись результатов в локальные таблицы](dev/streaming-query/table-writing.md).
- В [потоках изменений (CDC)](concepts/cdc.md) доступна выгрузка идентификаторов безопасности пользователя (`USER_SIDS`) — см. [`ALTER TABLE` `CHANGEFEED`](yql/reference/syntax/alter_table/changefeed.md).
- Для [внешних источников данных](concepts/datamodel/external_data_source.md) добавлен `AUTH_METHOD=IAM`.
...
- Доступны инструкции YQL [`BATCH UPDATE`](yql/reference/syntax/batch-update.md) и [`BATCH DELETE FROM`](yql/reference/syntax/batch-delete.md) для массового обновления и удаления данных в таблицах.
- Механизм выполнения операций записи существенно изменился -- теперь запись выполняется в потоковом режиме без полной материализации данных на стороне Query Processor перед отправкой в даташарды, что повышает производительность при крупных операциях записи. Изменение распространяется на часть сценариев; в отдельных случаях (например, таблицы со вторичными индексами) по-прежнему используется прежний подход. Общие сведения о конвейере выполнения см. в разделе [Выполнение запросов](concepts/query_execution/index.md).
- Оптимизировано выполнение Lookup Join: используется потоковый режим без материализации одной из сторон соединения, что снижает пиковое потребление памяти, ускоряет выполнение на больших наборах данных и снимает прежние ограничения на размер сторон соединения. См. описание [Index lookup Join](faq/yql.md#index-lookup-join) и синтаксис [оператора `JOIN`](yql/reference/syntax/select/join.md).
- Добавлена возможность выставлять права доступа для [системных представлений](devops/observability/system-views.md) кластера и баз данных.
- Для строковых таблиц доступна настройка [режимов кэширования](concepts/datamodel/table.md#cache-modes) и новый режим `in_memory`, который позволяет предзагружать данные таблицы в оперативную память при условии наличия необходимых объемов оперативной памяти.
- Для читателей топиков добавлен параметр [`availability-period`](reference/ydb-cli/topic-consumer-add.md), позволяющий продлить хранение неподтверждённых сообщений сверх retention-period
- Доступны [попартиционные метрики топиков и выгрузка в пользовательские шард-квоты](reference/observability/metrics/index.md#topics_partitions) для учёта и наблюдаемости.
- Ускорение запросов с `LIMIT` в колоночных таблицах за счёт раннего ограничения выборки на узлах хранения (для запросов без сортировки или с сортировкой по первичному ключу). Общий синтаксис [`LIMIT` и `OFFSET`](yql/reference/syntax/select/limit_offset.md) в YQL.
- Колоночные таблицы поддерживают тип `Bool` в схеме и запросах — см. [примитивные типы YQL](yql/reference/types/primitive.md#numeric).
- [Фильтруемый векторный индекс](dev/vector-indexes.md#filtered) корректно находит вставленные в таблицу после создания индекса строки с новыми значениями колонок фильтров.
- Потоковая обработка и поставка данных теснее интегрированы в ядро: [трансфер «топик → таблица»](concepts/transfer.md), [потоковые запросы](dev/streaming-query/index.md) стали доступны пользователям при включении 'EnableStreamingQueries'.
- Добавлена опция `overlap_clusters` для существенного улучшения качества векторного поиска за счёт помещения векторов в несколько кластеров индекса (настройки индекса) — см. [векторные индексы](dev/vector-indexes.md).
- Значительно ускорен поиск по всем видам векторных индексов за счёт подсчёта расстояний локально на каждом даташарде до передачи по сети — см. [VIEW (векторный индекс)](yql/reference/syntax/select/vector_index.md) и [векторные индексы](dev/vector-indexes.md).
- Ускорен полный векторный поиск без ANN-индекса за счёт pushdown (векторный поиск, KNN UDF) — см. [векторный поиск](concepts/query_execution/vector_search.md) и [модуль KNN](yql/reference/udf/list/knn.md).
- Полноценно поддержан механизм работы с секретами, хранимыми в базе данных: создание, изменение, удаление и использование — см. [Секреты](concepts/datamodel/secrets.md). Обратите внимание, что [старый синтаксис](concepts/datamodel/secrets.md) объявлен устаревшим.
- Улучшено выполнение [`UNION ALL`](yql/reference/syntax/select/union.md#union-all): теперь поддерживается параллельное выполнение, что повышает производительность аналитических запросов.

#### Исправления ошибок {#ispravleniya-oshibok1}

- [Исправлена](https://github.com/ydb-platform/ydb/pull/38425) уязвимость [LDAP-аутентификации](security/authentication.md): зная логин и пароль любого LDAP-пользователя (в том числе не входящего в группу с доступом к YDB), можно было обойти проверку членства в группе и получить доступ к кластеру (инъекция в LDAP-фильтр поиска пользователя; добавлено экранирование спецсимволов по RFC 2254).
...
- Добавлена поддержка конфигурации 2 ДЦ с синхронной записью данных (режим [`Bridge`](concepts/bridge.md)); доступно в YDB Enterprise.
...
- Реализованы [резервное копирование и восстановление](reference/ydb-cli/export-import/file-structure.md#topics) конфигурации топика в S3 и из S3.
...
- Увеличено [максимальное время на выполнение одного запроса](concepts/limits-ydb.md) с 30 минут до 2 часов.
...
- Включены по умолчанию:
  ...
  - системные представления с [историей перегруженных партиций](dev/system-views.md#top-overload-partitions).
...
- [Реализован](https://github.com/ydb-platform/ydb/pull/19504) [векторный индекс](dev/vector-indexes.md) для приближённого векторного поиска. Для векторного поиска опубликованы рецепты для [YDB CLI и YQL](recipes/vector-search/index.md), а также примеры работы [на С++ и Python](recipes/ydb-sdk/vector-search.md).
...
- Добавлен [механизм конфигурации V2](devops/configuration-management/configuration-v2/config-overview.md), упрощающий развёртывание новых кластеров YDB и дальнейшую работу с ними. [Сравнение](devops/configuration-management/compare-configs.md) механизмов конфигурации V1 и V2.
...
- Добавлены системные представления с информацией о [настройках прав доступа](dev/system-views.md#auth), [истории перегруженных партиций](dev/system-views.md#top-overload-partitions) - включается установкой флага `enable_followers_stats` в конфигурации кластера, [истории партиций строковых таблиц со сломанными блокировками (TLI)](dev/system-views.md#top-tli-partitions).
...
- Повышена безопасность учётных записей:
  ...
  - [Добавлена](https://github.com/ydb-platform/ydb/pull/11963) [проверка сложности пароля](reference/configuration/index.md#password-complexity) пользователя;
  - [Реализована](https://github.com/ydb-platform/ydb/pull/12578) [автоматическая блокировка пользователя](reference/configuration/index.md#account-lockout) при исчерпании лимита попыток ввода пароля;
  - [Добавлена](https://github.com/ydb-platform/ydb/pull/12983) возможность самостоятельной смены пароля пользователем.

- [Реализована](https://github.com/ydb-platform/ydb/issues/9748) возможность переключения функциональных флагов во время работы сервера YDB. Флаги, для которых в [proto-файле](https://github.com/ydb-platform/ydb/blob/main/ydb/core/protos/feature_flags.proto#L60) не указан параметр `(RequireRestart) = true`, будут применяться без рестарта кластера.
...
- [Реализована](https://github.com/ydb-platform/ydb/pull/15255) конвейерная обработка внутренних транзакций в [Node Broker](concepts/glossary.md#node-broker), что ускорило запуск динамических узлов в кластере YDB.
...
#### Производительность {#proizvoditelnost2}
...
- [Исправлены](https://github.com/ydb-platform/ydb/pull/18114) [проблемы](https://github.com/ydb-platform/ydb/issues/18071) с чтением сообщений больше 6Mb через [Kafka API](reference/kafka-api/index.md).
- [Устранена](https://github.com/ydb-platform/ydb/pull/18319) утечка памяти во время записи в [топик](concepts/glossary.md#topic).
...
#### Производительность {#proizvoditelnost3}
...
#### Производительность {#proizvoditelnost4}
...
- [Исправлена](https://github.com/ydb-platform/ydb/pull/12018) потенциальная поломка ["замороженных" блокировок](contributor/datashard-locks-and-change-visibility.md#vzaimodejstvie-s-raspredelyonnymi-tranzakciyami), к которой могли приводить массовые операции (например, удаление по TTL).
...
- [Исправлена](https://github.com/ydb-platform/ydb/pull/12448) [ошибка](https://github.com/ydb-platform/ydb/issues/12443), из-за которой [Health Check](reference/ydb-sdk/health-check-api.md) не сообщал о проблемах в синхронизации времени.
...
- [Поддержан](https://github.com/ydb-platform/ydb/pull/11276) рестарт без потери доступности кластера в [минимальной отказоустойчивой конфигурации](concepts/topology.md#reduced) из трех узлов.
...
- Добавлена [трассировка запросов](reference/observability/tracing/setup.md) – инструмент, позволяющий детально посмотреть путь следования запроса по распределенной системе.
- Добавлена поддержка [асинхронной репликации](concepts/async-replication.md), которая позволяет синхронизировать данные между базами YDB почти в реальном времени. Также она может быть использована для миграции данных между базами с минимальным простоем работающих с ними приложений.
- Добавлена поддержка [представлений (VIEW)](concepts/datamodel/view.md), которая может быть включена администратором кластера с помощью настройки `enable_views` в [динамической конфигурации](maintenance/manual/dynamic-config.md#obnovlenie-dinamicheskoj-konfiguracii).
- В [федеративных запросах](concepts/query_execution/federated_query/index.md) поддержаны новые внешние источники данных: MySQL, Microsoft SQL Server, Greenplum.
...
- Добавлен режим [автопартиционирования](concepts/datamodel/topic.md#autopartitioning) топиков, в котором топики могут разбивать партиции в зависимости от нагрузки с сохранением гарантий порядка чтения сообщений и exactly once записи. Режим может быть включен администратором кластера с помощью настроек `enable_topic_split_merge` и `enable_pqconfig_transactions_at_scheme_shard` в [динамической конфигурации](maintenance/manual/dynamic-config.md#obnovlenie-dinamicheskoj-konfiguracii).
- Добавлены [транзакции](concepts/transactions.md#topic-table-transactions) с участием [топиков](concepts/datamodel/topic.md) и строковых таблиц. Таким образом, можно транзакционно перекладывать данные из таблиц в топики и в обратном направлении, а также между топиками, чтобы данные не терялись и не дублировались. Транзакции могут быть включены администратором кластера с помощью настроек `enable_topic_service_tx` и `enable_pqconfig_transactions_at_scheme_shard` в [динамической конфигурации](maintenance/manual/dynamic-config.md#obnovlenie-dinamicheskoj-konfiguracii).
- [Добавлена](https://github.com/ydb-platform/ydb/pull/7150) поддержка [CDC](concepts/cdc.md) для синхронных вторичных индексов.
- Добавлена возможность изменить период хранения записей в [CDC](concepts/cdc.md) топиках.
- Добавлена поддержка [автоинкремента](yql/reference/types/serial.md) для колонок, включенных в первичный ключ таблицы.
- Добавлена запись в [аудитный лог](security/audit-log.md) событий логина пользователей в YDB, событий завершения сессии пользователя в пользовательском интерфейсе, а также запроса бэкапа и восстановления из бэкапа.
...
- Добавлена поддержка типа `Decimal` в [ключах партиционирования](dev/primary-key/column-oriented.md#klyuch-particionirovaniya).
- Улучшена диагностика проблем хранилища в HealthCheck.
- ***(Экспериментально)*** Добавлен [стоимостной оптимизатор](concepts/query_execution/optimizer.md#stoimostnoj-optimizator-zaprosov) для сложных запросов, где участвуют [колоночные таблицы](concepts/glossary.md#column-oriented-table). Оптимизатор рассматривает большое количество альтернативных планов выполнения и выбирает из них лучший на основе оценки стоимости каждого варианта. На текущий момент оптимизатор работает только с планами, где есть операции [JOIN](yql/reference/syntax/join.md).
- ***(Экспериментально)*** Реализована начальная версия [менеджера рабочей нагрузки](dev/resource-consumption-management.md), который позволяет создавать пулы ресурсов с ограничениями по процессору, памяти и количеству активных запросов. Реализованы классификаторы ресурсов для отнесения запросов к определенному пулу ресурсов.
- ***(Экспериментально)*** Реализован [автоматический выбор индекса](dev/secondary-indexes.md#avtomaticheskoe-ispolzovanie-indeksov-pri-vyborke) при выполнении запроса, который может быть включен администратором кластера с помощью настройки `index_auto_choose_mode` в `table_service_config` в [динамической конфигурации](maintenance/manual/dynamic-config.md#obnovlenie-dinamicheskoj-konfiguracii).
...
- [Добавлено](https://github.com/ydb-platform/ydb-embedded-ui/issues/929) обозначение [столбцов с автоинкрементом](yql/reference/types/serial.md).
- [Добавлена](https://github.com/ydb-platform/ydb-embedded-ui/pull/1438) вкладка с информацией о [таблетках](concepts/glossary.md#tablet).
- [Добавлена](https://github.com/ydb-platform/ydb-embedded-ui/pull/1289) вкладка с информацией о [группах распределенного хранилища](concepts/glossary.md#storage-group).
- [Добавлена](https://github.com/ydb-platform/ydb-embedded-ui/pull/1218) настройка для добавления [трассировки](reference/observability/tracing/setup.md) ко всем запросам и отображение результатов трассировки запроса.
...
#### Производительность {#proizvoditelnost5}
...
- [Реализован](https://github.com/ydb-platform/ydb/pull/11532) механизм, принудительно запускающий фоновый [компакшн](concepts/glossary.md#compaction) при обнаружении несоответствий между схемой данных и данными, хранящимися в [DataShard](concepts/glossary.md#data-shard). Это решает редко возникающую проблему задержки в изменении схемы данных.
...
- [Исключены](https://github.com/ydb-platform/ydb/pull/9934) удаленные индексы из подсчета лимита на [максимальное количество индексов](concepts/limits-ydb.md#schema-object).
...
- Добавлена возможность [задать приоритеты](devops/deployment-options/manual/maintenance.md) задачам обслуживания в [системе управления кластером](concepts/glossary.md#cms).
...
- [Исправлена](https://github.com/ydb-platform/ydb/pull/7709) [ошибка](https://github.com/ydb-platform/ydb/issues/7674) дублирования результатов SELECT-запроса при уменьшении квоты в [DataShard](concepts/glossary.md#data-shard).
- [Исправлены](https://github.com/ydb-platform/ydb/pull/6461) [ошибки](https://github.com/ydb-platform/ydb/issues/6220), возникающие при изменении состояния [координатора](concepts/glossary.md#coordinator).
- [Исправлены](https://github.com/ydb-platform/ydb/pull/5992) ошибки, возникающие в момент первичного сканирования [CDC](dev/cdc.md).
...
- [Исправлена](https://github.com/ydb-platform/ydb/pull/5993) редкая ошибка, из-за которой удаление по [TTL](concepts/ttl.md) приводило к аварийному завершению процесса.
- [Исправлена](https://github.com/ydb-platform/ydb/pull/5760) ошибка отображения статуса PDisk в интерфейсе [CMS](concepts/glossary.md#cms).
...
- [Исправлен](https://github.com/ydb-platform/ydb/pull/6695) учет свободной памяти в [interconnect](concepts/glossary.md#actor-system-interconnect).
...
### Производительность {#proizvoditelnost6}
...
### Производительность {#proizvoditelnost7}
...
- [Устранены](https://github.com/ydb-platform/ydb/pull/2126) утечки памяти, которые проявлялись при копировании данных топиков между кластерами YDB. Они могли приводить к завершению серверных процессов `ydbd` из-за исчерпания доступной оперативной памяти.
...
### Производительность {#proizvoditelnost8}
...
### Производительность {#proizvoditelnost9}
...
- Добавлен [аудитный лог](security/audit-log.md) — поток событий, который содержит информацию обо всех операциях над объектами YDB.

### Производительность {#proizvoditelnost10}
...
### Исправления ошибок и улучшения {#ispravleniya-oshibok-i-uluchsheniya}
...
### Что нового {#chto-novogo1}

- YDB Topics и Change Data Capture (CDC):

  - Представлен новый Topic API. [Топик](concepts/datamodel/topic.md) YDB — это сущность для хранения неструктурированных сообщений и доставки их различным подписчикам.
  - Поддержка нового Topic API добавлена в [YDB CLI](reference/ydb-cli/topic-overview.md) и [SDK](reference/ydb-sdk/topic.md). Topic API предоставляет методы потоковой записи и чтения сообщений, а также управления топиками.
