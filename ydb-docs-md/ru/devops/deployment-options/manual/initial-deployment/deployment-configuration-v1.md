...
url: "https://ydb.tech/docs/ru/devops/deployment-options/manual/initial-deployment/deployment-configuration-v1?version=v26.1"
doc_path: "ru/devops/deployment-options/manual/initial-deployment/deployment-configuration-v1"
version: "v26.1"
lang: "ru"
source_path: "ru/core/devops/deployment-options/manual/initial-deployment/deployment-configuration-v1.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/devops/deployment-options/manual/initial-deployment/deployment-configuration-v1.md"
...
Подготовьте конфигурационный файл YDB в зависимости от выбранной вами топологии (см. [выбор топологии](../../ansible/initial-deployment/deployment-preparation.md#topology-select)). Примеры для каждой поддерживаемой топологии приведены ниже во вкладках — выберите и используйте подходящий для вашего случая.
...
Для административных команд в разделах [Инициализируйте кластер](deployment-configuration-v1.md#initialize-cluster), [Создайте базу данных](deployment-configuration-v1.md#create-db), [Настройка учётных записей](deployment-configuration-v1.md#security-setup) и [Протестируйте работу с созданной базой](deployment-configuration-v1.md#try-first-db) в примерах используется путь `~/CA/certs/`. Чтобы не менять пути в командах, на каждом сервере создайте каталог `~/CA/certs/` в домашнем каталоге того пользователя, от имени которого вы выполняете эти команды, и поместите в него четыре файла: `ca.crt`, `node.crt`, `node.key` и `web.pem`.
...
- Вручную
  ...
  4. Запустите статический узел:
     ...
     При успешном старте в выводе появится, в частности, `Determined node ID: ...`. Если [проверка `web.pem`](deployment-configuration-v1.md#tls-copy-cert) не пройдена, здесь будет ошибка `Permission denied` для `/opt/ydb/certs/web.pem`.
...
После запуска статических узлов проверьте их работоспособность через встроенный веб-интерфейс YDB (Embedded UI):
...
Для инициализации кластера потребуется файл сертификата центра регистрации `ca.crt` из каталога `~/CA/certs/` (см. раздел [Скопируйте ключи и сертификаты TLS на каждый сервер](deployment-configuration-v1.md#tls-copy-cert)).
...
Для выполнения административной команды создания базы данных потребуется файл `ca.crt` из каталога `~/CA/certs/` (см. раздел [Скопируйте ключи и сертификаты TLS на каждый сервер](deployment-configuration-v1.md#tls-copy-cert)).
...
- Вручную
  ...
  4. Запустите динамический узел:
     ...
     Если [проверка `web.pem`](deployment-configuration-v1.md#tls-copy-cert) не пройдена, будет ошибка `Permission denied` для `/opt/ydb/certs/web.pem`.
...
В командах ниже `<node.ydb.tech>` — тот же FQDN динамического узла базы `/Root/testdb`, что в разделе [Настройка учетных записей](deployment-configuration-v1.md#security-setup).
