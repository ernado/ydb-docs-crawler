...
url: "https://ydb.tech/docs/ru/devops/deployment-options/manual/initial-deployment/deployment-preparation?version=v26.1"
doc_path: "ru/devops/deployment-options/manual/initial-deployment/deployment-preparation"
version: "v26.1"
lang: "ru"
...
Ознакомьтесь с [системными требованиями](../../../concepts/system-requirements.md) и [топологией кластера](../../../../concepts/topology.md).
...
Подробнее о требованиях к оборудованию описано в разделе [Системные требования и рекомендации для YDB](../../../concepts/system-requirements.md).
...
Для пакетной генерации или обновления сертификатов кластера YDB с помощью программного обеспечения OpenSSL можно воспользоваться [примером скрипта](https://github.com/ydb-platform/ydb/blob/main/ydb/deploy/tls_cert_gen/), размещённым в репозитории YDB на GitHub. Скрипт позволяет автоматически сформировать необходимые файлы ключей и сертификатов для всего набора узлов кластера за одну операцию, облегчая подготовку к установке.

## Создайте системного пользователя и группу, от имени которых будет работать YDB {#create-user}
...
Для корректной работы YDB, особенно при использовании [спиллинга](../../../../concepts/query_execution/spilling.md) в многоузловых кластерах, рекомендуется увеличить лимит на количество одновременно открытых файловых дескрипторов.
...
## Установите программное обеспечение YDB на каждом сервере {#install-binaries}

### Скачайте и распакуйте архив с исполняемым файлом `ydbd` и необходимыми для работы YDB библиотеками {#i-neobhodimymi-dlya-raboty-ydb-bibliotekami}
...
### Создайте на сервере директорию {#sozdajte-na-servere-direktoriyu}

```bash
sudo mkdir -p  /opt/ydb
```

### Скопируйте исполняемый файл и библиотеки в соответствующие директории {#skopirujte-ispolnyaemyj-fajl-i-biblioteki-v-sootvetstvuyushie-direktorii}
...
### Установите владельца файлов и каталогов {#ustanovite-vladelca-fajlov-i-katalogov}
...
В выводе `lsblk` выше системный диск — `vda`, диск для данных YDB — `vdb`. В командах ниже используется `/dev/vdb`; на вашем сервере подставьте путь к целому диску без номера раздела (например, `/dev/sda` или `/dev/nvme0n1`).
...
### Пример полной команды для разметки 3-х дисков {#primer-polnoj-komandy-dlya-razmetki-3-h-diskov}
...
### Проверьте подготовку дисков {#proverte-podgotovku-diskov}
