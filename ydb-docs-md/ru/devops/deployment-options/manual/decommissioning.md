...
4. Освободившееся оборудование из ЦОД-1 перемещается в ЦОД-2, его диски переводятся в состояние `DECOMMIT_NONE`.
...
Чтобы задать нужное состояние декомиссии диска, воспользуйтесь утилитой [YDB DSTool](../../../reference/ydb-dstool/index.md). Следующая команда задаст статус `DECOMMIT_IMMINENT` диску с ID `1000` на ноде с ID `1`:
