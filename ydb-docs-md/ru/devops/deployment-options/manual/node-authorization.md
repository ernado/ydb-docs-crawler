...
url: "https://ydb.tech/docs/ru/devops/deployment-options/manual/node-authorization?version=v26.1"
doc_path: "ru/devops/deployment-options/manual/node-authorization"
version: "v26.1"
lang: "ru"
source_path: "ru/core/devops/deployment-options/manual/node-authorization.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/devops/deployment-options/manual/node-authorization.md"
...
## Пререквизиты для настройки {#prerekvizity-dlya-nastrojki}

1. В развернутом кластере YDB должно быть [настроено шифрование трафика gRPC](../../../reference/configuration/tls.md#grpc) с использованием протокола TLS.
...
3. В параметры командной строки для [запуска узлов баз данных](initial-deployment/index.md#start-dynnode) необходимо добавить опции, задающие пути к файлам сертификатов доверенных центров сертификации, сертификата узла и ключа узла. Список дополнительных опций приведён в таблице ниже.
...
## Включение режима аутентификации и авторизации узлов {#vklyuchenie-rezhima-autentifikacii-i-avtorizacii-uzlov}
...
2. В блок настроек аутентификации кластера `security_config` добавьте элемент `register_dynamic_node_allowed_sids`, указав список субъектов доступа, которым разрешена регистрация узлов баз данных. Пример:
   ...
   Более подробная информация по настройке параметров аутентификации кластера приведена в [соответствующем разделе документации](../../../reference/configuration/security_config.md#security-access-levels).

3. В блоке настроек `auth_config` присвойте параметру `node_registration_token` пустую строку. Пример:
...
4. Обновите файлы статической конфигурации на всех узлах кластера вручную либо с помощью [плейбука Ansible](../ansible/update-config.md).

5. Выполните поэтапный перезапуск узлов хранения кластера [с помощью ydbops](../../../reference/ydbops/rolling-restart-scenario.md) либо с использованием [плейбука Ansible](../ansible/restart.md).

6. Выполните поэтапный перезапуск узлов баз данных кластера с помощью ydbops либо с использованием плейбука Ansible.
