---
title: "ru/devops/deployment-options/manual/bridge-management"
url: "https://ydb.tech/docs/ru/devops/deployment-options/manual/bridge-management?version=v26.1"
doc_path: "ru/devops/deployment-options/manual/bridge-management"
version: "v26.1"
lang: "ru"
source_path: "ru/core/devops/deployment-options/manual/bridge-management.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/devops/deployment-options/manual/bridge-management.md"
description: "Управление кластером в режиме bridge. Функциональность Корпоративной СУБД Яндекса."
revision: "e9f541853a7760e5c0d0babc071d86df7f523cf5"
---

# ru/devops/deployment-options/manual/bridge-management

## Управление кластером в режиме bridge {#upravlenie-klasterom-v-rezhime-bridge}
...
Ниже приведены типовые операции для кластера в [режиме bridge](../../../concepts/bridge.md) с использованием [соответствующих команд YDB CLI](../../../reference/ydb-cli/commands/bridge/index.md).
...
Если отключается текущий `PRIMARY` и не было возможности [сменить его заранее](bridge-management.md#switchover), то эти операции можно совместить, указав новый `PRIMARY` в аргументе `--new-primary`, который должен быть в состоянии `SYNCHRONIZED`.
...
> [!WARNING]
> Перед началом плановых работ обязательно убедитесь через команду [list](bridge-management.md#list), что операция вывода pile из кластера завершилась успешно и все pile находятся в ожидаемом состоянии.
...
Сразу после запуска операции pile переходит в состояние `NOT_SYNCHRONIZED` и запускается фоновый процесс синхронизации данных; по её завершении pile автоматически становится `SYNCHRONIZED`. Дождавшись этого состояния, при необходимости можно [переключить роль `PRIMARY` на данный pile](bridge-management.md#switchover).
