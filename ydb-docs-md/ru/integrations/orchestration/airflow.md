...
url: "https://ydb.tech/docs/ru/integrations/orchestration/airflow?version=v26.1"
doc_path: "ru/integrations/orchestration/airflow"
version: "v26.1"
lang: "ru"
source_path: "ru/core/integrations/orchestration/airflow.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/integrations/orchestration/airflow.md"
description: "Интеграция YDB с Apache Airflow™ позволяет автоматизировать сложные рабочие процессы и управлять ими. Apache Airflow™ предоставляет возможности для планирования"
revision: "e9f541853a7760e5c0d0babc071d86df7f523cf5"
---

# Apache Airflow™

Интеграция YDB с [Apache Airflow™](https://airflow.apache.org) позволяет автоматизировать сложные рабочие процессы и управлять ими. Apache Airflow™ предоставляет возможности для планирования задач, мониторинга их выполнения и управления зависимостями между ними — оркестрацией. Использование Airflow для оркестрации задач, таких как загрузка данных в YDB, выполнение запросов и управление транзакциями, позволяет автоматизировать и оптимизировать операционные процессы. Это особенно важно для задач ETL, где данные больших объемов требуют регулярного извлечения, преобразования и загрузки.

Для работы под управлением Apache Airflow™ YDB предоставляет пакет [провайдера](https://airflow.apache.org/docs/apache-airflow-providers) [apache-airflow-providers-ydb](https://pypi.org/project/apache-airflow-providers-ydb/). [Задания Apache Airflow™](https://airflow.apache.org/docs/apache-airflow/stable/index.html) представляют собой приложения на языке Python, состоящие из набора [операторов Apache Airflow™](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/operators.html) и их [зависимостей](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/dags.html), определяющих порядок выполнения.
...
- Оператор [YDBExecuteQueryOperator](airflow.md#ydb_execute_query_operator) для интеграции задач в планировщик Apache Airflow™.
- Хук [YDBHook](airflow.md#ydb_hook) для прямого взаимодействия с YDB.
...
- `ydb_conn_id` — идентификатор подключения с типом `YDB`, содержащий параметры соединения с YDB. Если не указан, то используется соединение с именем [`ydb_default`](airflow.md#ydb_default). Соединение `ydb_default` предустанавливается в составе Apache Airflow™, отдельно его заводить не нужно.
...
- [bulk_upsert](airflow.md#bulk_upsert);
- [get_conn](airflow.md#get_conn).

#### bulk_upsert {#bulk_upsert}

Выполняет [пакетную вставку данных](../../recipes/ydb-sdk/bulk-upsert.md) в таблицы YDB.
...
## Подключение к YDB {#ydb_default}

Для подключения к YDB необходимо создать новое или отредактировать существующее [подключение Apache Airflow™](https://airflow.apache.org/docs/apache-airflow/stable/howto/connection.html) с типом `YDB`.

![](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/integrations/orchestration/_assets/ydb_connection.png)
...
## Соответствие YQL и Python-типов {#sootvetstvie-yql-i-python-tipov}
...
![](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/integrations/orchestration/_assets/airflow_dag.png)

Для выполнения запросов к базе данных YDB используется предварительно созданное соединение c YDB типа [YDB Connection](https://airflow.apache.org/docs/apache-airflow-providers-ydb/stable/connections/ydb.html) c именем `test_ydb_connection`.
