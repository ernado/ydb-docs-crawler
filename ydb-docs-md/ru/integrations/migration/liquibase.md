---
title: "Миграции схемы данных YDB с помощью Liquibase"
url: "https://ydb.tech/docs/ru/integrations/migration/liquibase?version=v26.1"
doc_path: "ru/integrations/migration/liquibase"
version: "v26.1"
lang: "ru"
source_path: "ru/core/integrations/migration/liquibase.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/integrations/migration/liquibase.md"
description: "Введение."
revision: "95f7629e80402dd261127ed00cdc781d2b8433de"
---

# Миграции схемы данных YDB с помощью Liquibase

## Введение {#introduction}

[Liquibase](https://www.liquibase.com/) – это библиотека с открытым исходным кодом для отслеживания, управления и применения изменений схемы базы данных. Она расширяется диалектами для различных систем управления базами данных (СУБД), включая YDB.
...
### Создание таблицы {#sozdanie-tablicy}
...
Таблица сравнения описаний типов Liquibase с [типами YDB](../../yql/reference/types/primitive.md):

| Liquibase типы | YDB тип |
| --- | --- |
...
### Изменение структуры таблицы {#izmenenie-struktury-tablicy}
...
### Ввод данных в таблицу {#vvod-dannyh-v-tablicu}

`loadData`, `loadUpdateData` - загрузка данных из `CSV` файла. `loadUpdateData` загружает данные командой [UPSERT INTO](../../yql/reference/syntax/upsert_into.md).
...
Вы также можете указать любое значение в поле `value`. Данные из поля `value` в наборе изменений `insert` или `CSV` файлов будут автоматически преобразованы в требуемые типы с учетом строгой типизации в YDB.
...
| `Timestamp` | Временная метка из стандарта [ISO-8601](https://ru.wikipedia.org/wiki/ISO_8601) соответствует классу `java.time.Instant` в Java, timezone будет установлена `UTC` (точность в микросекундах - ограничение типа данных `Timestamp` в YDB) |
...
> [!WARNING]
> Чтобы понять, какие SQL-конструкции может выполнять YDB и какие существуют ограничения на типы данных, ознакомьтесь с документацией по языку запросов [YQL](../../yql/reference/index.md).
...
- Liquibase CLI
  ...
  Теперь утилиту командной строки liquibase можно использовать с YDB.

{% endlist %}

## Сценарии использования liquibase {#scenarii-ispolzovaniya-liquibase}

### Инициализация liquibase на пустой YDB {#inicializaciya-liquibase-na-pustoj-ydb}

Основной командой является `liquibase update`, которая применяет миграции, если текущая схема YDB отстает от пользовательского описания.
...
![_assets/liquibase-step-1.png](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/integrations/migration/_assets/liquibase-step-1.png)
...
### Эволюция схемы базы данных {#evolyuciya-shemy-bazy-dannyh}

Допустим нам нужно создать YDB топик и выключить параметр `AUTO_PARTITIONING_BY_SIZE` таблицы. Это можно сделать нативным SQL скриптом:
...
![_assets/liquibase-step-2.png](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/integrations/migration/_assets/liquibase-step-2.png)

### Инициализация liquibase в проекте с непустой схемой данных {#inicializaciya-liquibase-v-proekte-s-nepustoj-shemoj-dannyh}

Предположим, что я имею существующий проект с текущей схемой базы данных:

![_assets/liquibase-step-3.png](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/integrations/migration/_assets/liquibase-step-3.png)
...
![_assets/liquibase-step-4.png](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/integrations/migration/_assets/liquibase-step-4.png)

## Подключение к YDB {#connect-to-ydb}
