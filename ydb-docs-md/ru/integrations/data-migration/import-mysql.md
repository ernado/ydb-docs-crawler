---
title: "Импорт структуры таблиц и данных из MySQL"
url: "https://ydb.tech/docs/ru/integrations/data-migration/import-mysql?version=v26.1"
doc_path: "ru/integrations/data-migration/import-mysql"
version: "v26.1"
lang: "ru"
source_path: "ru/core/integrations/data-migration/import-mysql.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/integrations/data-migration/import-mysql.md"
...
| Таблица без `PRIMARY KEY` | Не поддерживается при автоматическом создании схемы — см. [Таблицы без первичного ключа](import-mysql.md#tables-without-pk) |
...
Если нужен автоматический импорт без ключа в MySQL, рассмотрите [YDB Importer](import-mysql.md#ydb-importer-comparison) — он добавляет синтетическую колонку `ydb_synth_key` (SHA-256 по строке). mysql2ydb синтетические ключи не создаёт.
...
- **Копия схемы один к одному** — исходные имена таблиц и колонок; сопоставление типов MySQL с ближайшими типами YDB (`AUTO_INCREMENT` → `BigSerial`, `UNSIGNED` → `Uint32`/`Uint64`, вторичные индексы, `TINYINT(1)` → `Bool`; см. раздел [Соответствие схемы MySQL](import-mysql.md#schema-fidelity)).
...
>
> Таблицы без `PRIMARY KEY` в MySQL **не поддерживаются** при автоматическом создании схемы. Перед запуском добавьте первичный ключ в MySQL или подготовьте схему вручную и используйте `-data-only` (см. раздел [Таблицы без первичного ключа](import-mysql.md#tables-without-pk)).
...
- Если в MySQL нет `PRIMARY KEY`, но схема в YDB уже создана вручную (режим `-data-only`), применяется `LIMIT batch_size OFFSET offset` — см. раздел [Таблицы без первичного ключа](import-mysql.md#tables-without-pk).

## Запись в YDB {#bulkupsert}
...
Запросы к YDB помечены как идемпотентные, поэтому SDK автоматически повторяет их при сетевых сбоях. Отсутствие дубликатов при перезапуске миграции обеспечивается перезаписью строк по ключу и сохранением прогресса в служебной таблице (см. раздел [Возможности](import-mysql.md#features), пункт «Возобновление»).

## Сравнение с YDB Importer {#ydb-importer-comparison}

Для загрузки данных в YDB есть и другие инструменты. [YDB Importer](import-jdbc.md) — универсальный JDBC-импортёр для PostgreSQL, Oracle, SQL Server и других источников: XML-конфигурация, параллельный импорт, настройка партиционирования.
