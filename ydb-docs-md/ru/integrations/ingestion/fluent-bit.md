---
title: "Сбор логов с помощью FluentBit"
url: "https://ydb.tech/docs/ru/integrations/ingestion/fluent-bit?version=v26.1"
doc_path: "ru/integrations/ingestion/fluent-bit"
version: "v26.1"
lang: "ru"
...
В разделе представлена реализация интеграции между YDB и инструментом захвата логов [FluentBit](https://fluentbit.io) с целью сохранения данных логов в YDB для последующего просмотра или анализа.

## Введение {#vvedenie}

FluentBit – инструмент, который может собирать текстовые данные, манипулировать ими (изменять, преобразовывать, объединять) и отправлять в различные хранилища для дальнейшей обработки. Для поставки логов в таблицы YDB с помощью FluentBit разработана библиотека-плагин. Исходный код библиотеки доступен [здесь](https://github.com/ydb-platform/fluent-bit-ydb).
...
## Создание таблиц для хранения логов {#sozdanie-tablic-dlya-hraneniya-logov}
...
## Развертывание FluentBit и настройка сбора логов {#razvertyvanie-fluentbit-i-nastrojka-sbora-logov}
...
Плагин для поддержки YDB в FluentBit доступен в [репозитории](https://github.com/ydb-platform/fluent-bit-ydb) вместе с инструкциями по сборке. Для установки в формате контейнера предусмотрен Docker-образ `ghcr.io/ydb-platform/fluent-bit-ydb`.
...
## Настройка записи логов в YDB {#nastrojka-zapisi-logov-v-ydb}
...
| `Match` | (опционально) [Выражение для отбора тегов](https://docs.fluentbit.io/manual/concepts/data-pipeline/router) записей, направляемых в YDB |
...
## Сбор логов в кластере Kubernetes {#sbor-logov-v-klastere-kubernetes}
...
![FluentBit in Kubernetes cluster](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/integrations/ingestion/_assets/fluent-bit-ydb-scheme.png)
...
### Таблица для хранения логов Kubernetes {#tablica-dlya-hraneniya-logov-kubernetes}
...
### Конфигурация FluentBit {#konfiguraciya-fluentbit}
...
### Развертывание FluentBit в кластере Kubernetes {#razvertyvanie-fluentbit-v-klastere-kubernetes}
...
### Проверка установки {#proverka-ustanovki}
...
### Очистка ресурсов {#ochistka-resursov}
