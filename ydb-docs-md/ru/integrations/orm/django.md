---
title: "Django"
url: "https://ydb.tech/docs/ru/integrations/orm/django?version=v26.1"
doc_path: "ru/integrations/orm/django"
version: "v26.1"
lang: "ru"
source_path: "ru/core/integrations/orm/django.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/integrations/orm/django.md"
description: "Django — это популярный Python веб-фреймворк с мощной ORM для работы с базами данных."
...
### Настройки подключения {#nastrojki-podklyucheniya}

Для подключения к YDB используйте стандартный механизм настройки баз данных Django через переменную `DATABASES` в файле `settings.py`. Подробнее о конфигурации баз данных в Django можно прочитать в официальной [документации](https://docs.djangoproject.com/en/stable/ref/settings/#databases).
...
- `HOST` — адрес сервера YDB (например, `localhost`).
- `PORT` — порт gRPC YDB (по умолчанию 2136).
- `DATABASE` — абсолютный путь к базе данных YDB (например, `/local` для локальных тестов или `/my_production_db` для production).
...
### Методы аутентификации {#metody-autentifikacii}
...
### Использование шифрования {#ispolzovanie-shifrovaniya}
...
## Поля Django {#polya-django}

YDB backend поддерживает встроенные поля Django.

### Поддерживаемые поля Django {#podderzhivaemye-polya-django}
...
### Необходимые условия {#neobhodimye-usloviya}

Перед началом работы убедитесь, что:

1. Развернут кластер YDB [локально](../../quickstart.md) или [используя Ansible](../../devops/deployment-options/ansible/initial-deployment/index.md)
...
### Перейдите в рабочую директорию {#perejdite-v-rabochuyu-direktoriyu}

```bash
cd examples/bookstore
```

### Конфигурация подключения к YDB {#konfiguraciya-podklyucheniya-k-ydb}
...
### Запустите миграции {#zapustite-migracii}

```bash
python manage.py migrate
```

### Запустите сервер {#zapustite-server}

```bash
python manage.py runserver
```

### Проверьте работу приложения {#proverte-rabotu-prilozheniya}
