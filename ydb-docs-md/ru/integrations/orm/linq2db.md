---
title: "Провайдер YDB для LinqToDB"
url: "https://ydb.tech/docs/ru/integrations/orm/linq2db?version=v26.1"
doc_path: "ru/integrations/orm/linq2db"
version: "v26.1"
lang: "ru"
source_path: "ru/core/integrations/orm/linq2db.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/integrations/orm/linq2db.md"
description: "Введение. Это руководство по использованию LinqToDB с YDB."
...
Это руководство по использованию [LinqToDB](https://linq2db.github.io/) с YDB.

linq2db — лёгкий и быстрый ORM для .NET, предоставляющий типобезопасные [LINQ](https://learn.microsoft.com/en-us/dotnet/csharp/linq/) запросы и точный контроль над SQL. Провайдер YDB формирует корректный YQL, поддерживает типы YDB, генерацию схемы и массовые операции (Bulk Copy).

## Установка провайдера YDB {#install-provider}
...
### Таблица соответствия .NET типов с типами YDB {#types}
...
| `decimal` | `Decimal` | `Decimal(p,s)` | По умолчанию — Decimal(22,9). Чтобы задать свои Precision/Scale, используйте Decimal(p,s). [Пример](linq2db.md#пример-кастомного-decimal) |
| `float` | `Single` | `Float` | — |
...
| `DateOnly` / `DateTime` | `Date` | `Date32` | Более широкий диапазон дат. Укажите DbType = "Date32". [Пример](linq2db.md#пример-кастомного-decimal) |
| `DateTime` | `DateTime` | `Datetime64` | Точность до секунд, расширенный диапазон. Укажите DbType = "Datetime64". [Пример](linq2db.md#пример-кастомного-decimal) |
| `DateTime` / `DateTimeOffset` | `DateTime2` | `Timestamp64` | Точность до микросекунд, расширенный диапазон. Укажите DbType = "Timestamp64". [Пример](linq2db.md#пример-кастомного-decimal) |
| `TimeSpan` | `Interval` | `Interval64` | Более широкий диапазон интервалов. Укажите DbType = "Interval64". [Пример](linq2db.md#пример-кастомного-decimal) |
...
### Пример кастомного Decimal {#primer-kastomnogo-decimal}
...
### Пример кастомного DbType {#primer-kastomnogo-dbtype}
...
### Генерация схемы из атрибутов {#generaciya-shemy-iz-atributov}
...
### Эволюция схемы (добавление поля) {#evolyuciya-shemy-dobavlenie-polya}
...
### Индексы YDB: как задать параметры {#indeksy-ydb-kak-zadat-parametry}
...
#### Вариант A — через атрибут (имя индекса) {#variant-a-—-cherez-atribut-imya-indeksa}
...
#### Вариант B — расширенные параметры (ASYNC, COVER) отдельной командой {#variant-b-—-rasshirennye-parametry-async,-cover-otdelnoj-komandoj}
...
### Связи между сущностями {#svyazi-mezhdu-sushnostyami}
...
### Примеры сгенерированного YQL для выборок по связям {#primery-sgenerirovannogo-yql-dlya-vyborok-po-svyazyam}
...
### Пример «прикладной» сущности и генерируемого DDL {#primer-«prikladnoj»-sushnosti-i-generiruemogo-ddl}
...
### Массовые операции: вставка, обновление и удаление {#massovye-operacii-vstavka,-obnovlenie-i-udalenie}

#### Массовая вставка (BulkCopy) {#massovaya-vstavka-bulkcopy}
