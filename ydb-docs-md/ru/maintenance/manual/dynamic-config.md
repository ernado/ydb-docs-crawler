---
title: "Динамическая конфигурация кластера"
url: "https://ydb.tech/docs/ru/maintenance/manual/dynamic-config?version=v26.1"
doc_path: "ru/maintenance/manual/dynamic-config"
version: "v26.1"
lang: "ru"
source_path: "ru/core/devops/configuration-management/configuration-v1/dynamic-config.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/devops/configuration-management/configuration-v1/dynamic-config.md"
description: "Динамическая конфигурация позволяет запускать динамические узлы, сконфигурировав их централизованно, без необходимости раскладывать файлы по узлам вручную. YDB"
revision: "95f7629e80402dd261127ed00cdc781d2b8433de"
---

# Динамическая конфигурация кластера

Динамическая конфигурация позволяет запускать динамические [узлы](../../concepts/glossary.md#node), сконфигурировав их централизованно, без необходимости раскладывать файлы по узлам вручную. YDB выступает в роли системы управления конфигурациями, предоставляя инструменты для надёжного хранения, версионирования и доставки конфигурации, а так же [DSL (Domain Specific Language)](../../devops/configuration-management/configuration-v1/dynamic-config-selectors.md) для переопределения её частей для определённых групп узлов. Конфигурация представляет собой YAML документ. Он является расширенной версией статической конфигурации:
...
Эта конфигурация загружается в кластер, где надёжно сохраняется и доставляется до каждого динамического узла при его старте. [Некоторые настройки](../../devops/configuration-management/configuration-v1/dynamic-config.md#dynamic-kinds) обновляются на лету, без перезапуска узлов. С помощью динамической конфигурации можно централизованно решить следующие задачи:
...
1. Включите в кластере [аутентификацию и авторизацию узлов баз данных](../../devops/configuration-management/configuration-v1/node-authorization.md).

2. Если ранее в кластере использовалось [управление конфигурацией через CMS](../../devops/configuration-management/configuration-v1/cms.md), выгрузите существующие настройки в формате YAML. Для этого выполните следующую команду:
...
3. Сформируйте первоначальный файл динамической конфигурации:
   ...
   - Если ранее в CMS выполнялись настройки (экспортированные на предыдущем шаге), используйте полученный файл в качестве основы и:
     ...
     - добавьте секцию `metadata` по образцу из [примера конфигурации](../../devops/configuration-management/configuration-v1/dynamic-config.md#example);
     - в секцию `config` добавьте параметр `yaml_config_enabled: true`.

   - Если ранее настройки через CMS не выполнялись, используйте [минимальное наполнение](../../devops/configuration-management/configuration-v1/dynamic-config.md#example) файла динамической конфигурации.
...
Подробнее параметры конфигурации описаны на странице [Параметры конфигурации кластера](../../reference/configuration/index.md).
...
## Обновление динамической конфигурации {#obnovlenie-dinamicheskoj-konfiguracii}
...
Дополнительные возможности конфигурирования описаны на страницах [селекторы](../../devops/configuration-management/configuration-v1/dynamic-config-selectors.md) и [временная конфигурация](../../devops/configuration-management/configuration-v1/dynamic-config-volatile-config.md).  
 Все команды для работы с конфигурацией описаны в разделе [Получить конфигурацию кластера](../../reference/ydb-cli/configs.md).

## Механизм работы {#mehanizm-raboty}

### Обновление конфигурации c точки зрения администратора {#obnovlenie-konfiguracii-c-tochki-zreniya-administratora}

1. Конфигурационный файл загружается пользователем при помощи [grpc-вызова](https://github.com/ydb-platform/ydb/blob/5251c9ace0a7617c25d50f1aa4d0f13e3d56f985/ydb/public/api/grpc/draft/ydb_dynamic_config_v1.proto#L22) или [YDB CLI](../../reference/ydb-cli/index.md) в кластер.
...
### Обновление конфигурации с точки зрения узла кластера {#obnovlenie-konfiguracii-s-tochki-zreniya-uzla-klastera}
...
### Версионирование конфигурации {#versionirovanie-konfiguracii}
...
- Использование более 30 различных [лейблов](../../devops/configuration-management/configuration-v1/dynamic-config-selectors.md) в [селекторах](../../devops/configuration-management/configuration-v1/dynamic-config-selectors.md) может привести к задержкам при валидации конфигурации в десятки секунд, т.к. YDB необходимо проверить валидность каждой возможной конечной конфигурации. При этом количество значений одного лейбла влияет намного меньше.
