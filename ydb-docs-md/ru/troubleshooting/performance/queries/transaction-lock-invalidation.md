---
title: "Инвалидация блокировок транзакций"
url: "https://ydb.tech/docs/ru/troubleshooting/performance/queries/transaction-lock-invalidation?version=v26.1"
doc_path: "ru/troubleshooting/performance/queries/transaction-lock-invalidation"
version: "v26.1"
lang: "ru"
source_path: "ru/core/troubleshooting/performance/queries/transaction-lock-invalidation.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/troubleshooting/performance/queries/transaction-lock-invalidation.md"
description: "Инвалидация блокировок транзакций (TLI) происходит, когда одна транзакция (нарушитель) записывает данные и ломает оптимистичные блокировки другой транзакции (же"
...
> [!NOTE]
> YDB SDK предоставляет встроенный механизм повтора временных ошибок. Подробнее см. [Обработка ошибок](../../../reference/ydb-sdk/error_handling.md).

## Предотвращение конфликтов {#predotvrashenie-konfliktov}
...
### Пример: конфликт транзакций при резервировании товара {#primer-konflikt-tranzakcij-pri-rezervirovanii-tovara}
...
### Мониторинг в Grafana {#monitoring-v-grafana}
...
2. Проверьте, есть ли всплески количества ошибок на диаграмме **Transaction Lock Invalidation**.
   ...
   ![](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/troubleshooting/performance/queries/_assets/transactions-locks-invalidation.png)

   Эта диаграмма отображает количество запросов в секунду, возвращаемых с ошибкой «transaction locks invalidated».

### Диагностика через логи TLI {#diagnostika-cherez-logi-tli}
...
По этому `VictimQuerySpanId` можно найти в логах сервера полный контекст конфликта: какой запрос установил блокировки и какой их сломал. Подробнее о включении логирования, формате записей и корреляции событий, а также об утилите `find_tli_chain` для автоматического анализа логов см. в [Логирование TLI](tli-logging.md).

### Анализ через системные представления {#analiz-cherez-sistemnye-predstavleniya}

Для анализа конфликтов блокировок доступны следующие системные представления:

#### Анализ на уровне запросов {#analiz-na-urovne-zaprosov}
...
#### Анализ на уровне партиций {#analiz-na-urovne-particij}
