---
title: "Форматы данных при чтении/записи из топиков"
url: "https://ydb.tech/docs/ru/dev/streaming-query/streaming-query-formats?version=v26.1"
doc_path: "ru/dev/streaming-query/streaming-query-formats"
version: "v26.1"
lang: "ru"
...
В данном разделе описываются форматы данных [потоковых запросов](../../concepts/glossary.md#streaming-query), поддерживаемые в YDB при чтении из топиков, и список поддерживаемых [YQL типов](../../yql/reference/types/index.md) для каждого формата данных.
...
[Примеры парсинга данных в пользовательских форматах](streaming-query-formats.md#parsing).

В примерах на этой странице `ydb_source` — это заранее созданный [внешний источник данных](../../concepts/datamodel/external_data_source.md), `input_topic` и `output_topic` — топики, доступные через него, а `output_table` — таблица YDB.
...
Данный формат основан на [JSON-представлении](https://ru.wikipedia.org/wiki/JSON) данных. В этом формате каждое сообщение должно представлять собой JSON-объект. Такой формат используется при передаче данных через потоковые системы, например, Apache Kafka или [Топики YDB](../../concepts/datamodel/topic.md).  
...
В этом формате схема читаемых данных должна состоять только из одной колонки с одним из разрешённых типов данных, подробнее см. [ниже](streaming-query-formats.md#schema).
...
Данный формат позволяет считывать содержимое сообщений как есть, в "сыром" виде. Считанные таким образом данные можно обработать средствами [YQL](../../yql/reference/udf/list/string.md). Cхема по умолчанию: `SCHEMA(Data String)`.
