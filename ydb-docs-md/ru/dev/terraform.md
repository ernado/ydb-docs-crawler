---
title: "Управление YDB с помощью Terraform"
url: "https://ydb.tech/docs/ru/dev/terraform?version=v26.1"
doc_path: "ru/dev/terraform"
version: "v26.1"
lang: "ru"
...
С помощью [Terraform](https://www.terraform.io/) можно создавать, удалять и изменять следующие объекты внутри кластера YDB:
...
4. Установить и настроить [Terraform provider for YDB](https://github.com/ydb-platform/terraform-provider-ydb/)

## Настройка Terraform провайдера для работы с YDB {#setup}
...
## Использование Terraform провайдера YDB {#work-with-tf}
...
- `database_endpoint` - используется при работе с ресурсом [топиков](terraform.md#topic_resource) (аналог `connection_string` при работе с ресурсами строковых таблиц).
...
Если вы используете создание ресурсов `ydb_table_changefeed` или `ydb_topic` и на сервере YDB не включена авторизация, то в конфиге БД [config.yaml](../reference/configuration/index.md) нужно указать:
...
### Пример использования всех типов ресурсов YDB Terraform провайдера {#primer-ispolzovaniya-vseh-tipov-resursov-ydb-terraform-provajdera}
...
- `connection_string` — (обязательный) [строка соединения](terraform.md#connection_string).
- `column` — (обязательный) свойства колонки (см. аргумент [column](terraform.md#column)).
- `family` — (необязательный) группа колонок (см. аргумент [family](terraform.md#family)).
...
- `ttl` — (необязательный) TTL (см. аргумент [ttl](terraform.md#ttl)).
- `partitioning_settings` — (необязательный) настройки партицирования (см. аргумент [partitioning_settings](terraform.md#partitioning-settings)).
...
- `family` — (необязательный) имя группы колонок (см. аргумент [family](terraform.md#family)).
...
- `connection_string` — [строка соединения](terraform.md#connection_string). Указывается, если не задан `table_id`.
...
- `mode` — (обязательный) режим работы [потока изменений](../yql/reference/syntax/alter_table/index.md#changefeed-options).
- `format` — (обязательный) формат [потока изменений](../yql/reference/syntax/alter_table/index.md#changefeed-options).
...
- `consumer` — (необязательный) читатель потока изменений (см. аргумент [#consumer](terraform.md#consumer)).
...
- `starting_message_timestamp_ms` — (необязательный) временная метка в формате [UNIX timestamp](https://ru.wikipedia.org/wiki/Unix-%D0%B2%D1%80%D0%B5%D0%BC%D1%8F) в миллисекундах, с которой читатель начнет читать данные.

### Примеры использования {#primery-ispolzovaniya-{manage-examples}}
...
## Управление конфигурацией топиков YDB через Terraform {#upravlenie-konfiguraciej-topikov-ydb-cherez-terraform}

Для работы с [топиками](../concepts/datamodel/topic.md) используется ресурс `ydb_topic`
...
- `starting_message_timestamp_ms` - временная метка в формате [UNIX timestamp](https://ru.wikipedia.org/wiki/Unix-%D0%B2%D1%80%D0%B5%D0%BC%D1%8F) в миллисекундах, с которой читатель начнёт читать данные, по умаолчанию - 0, то есть с начала поставки.
