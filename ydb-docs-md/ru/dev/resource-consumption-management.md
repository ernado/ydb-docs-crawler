---
title: "Workload Manager — управление потреблением ресурсов"
url: "https://ydb.tech/docs/ru/dev/resource-consumption-management?version=v26.1"
doc_path: "ru/dev/resource-consumption-management"
version: "v26.1"
lang: "ru"
...
## Создание пула ресурсов {#sozdanie-pula-resursov}
...
Ознакомиться с полным перечнем параметров пулов ресурсов можно в справке по [Параметры](../yql/reference/syntax/create-resource-pool.md#parameters). Часть параметров является глобальными для всей базы данных (например, `CONCURRENT_QUERY_LIMIT`, `QUEUE_SIZE`, `DATABASE_LOAD_CPU_THRESHOLD`), а другие — применяются только к одному вычислительному узлу (например, `QUERY_CPU_LIMIT_PERCENT_PER_NODE`, `TOTAL_CPU_LIMIT_PERCENT_PER_NODE`, `QUERY_MEMORY_LIMIT_PERCENT_PER_NODE`). Между всеми пулами может разделяться CPU в случае переподписки на одном вычислительном узле с помощью `RESOURCES_WEIGHT`.

![resource_pools](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/resource_pool.png)

Рассмотрим на примере выше, что на самом деле означают эти параметры и как они будут влиять на распределение ресурсов. Допустим, в базе данных YDB выделено 101010 узлов по 10vCPU10 vCPU10vCPU. В сумме в такой базе данных 100vCPU100 vCPU100vCPU. Тогда на каждом узле для пула ресурсов с именем `olap` будет выделено:

10vCPU⋅TOTAL_CPU_LIMIT_PERCENT_PER_NODE100=10vCPU⋅0.7=7vCPU\\frac{10 vCPU \\cdot TOTAL\\_CPU\\_LIMIT\\_PERCENT\\_PER\\_NODE}{100} = 10 vCPU \\cdot 0.7 = 7 vCPU10010vCPU⋅TOTAL_CPU_LIMIT_PERCENT_PER_NODE​=10vCPU⋅0.7=7vCPU

В сумме, при равномерном распределении ресурсов на всю базу данных, пулу ресурсов будет выделено:

7vCPU⋅10 (nodes)=70vCPU7 vCPU \\cdot 10 \\text{ (nodes)} = 70 vCPU7vCPU⋅10 (nodes)=70vCPU

На один запрос в этом пуле ресурсов будет выделено:

10vCPU⋅TOTAL_CPU_LIMIT_PERCENT_PER_NODE100⋅QUERY_CPU_LIMIT_PERCENT_PER_NODE100=10vCPU⋅0.7⋅0.5=3.5vCPU\\frac{10 vCPU \\cdot TOTAL\\_CPU\\_LIMIT\\_PERCENT\\_PER\\_NODE}{100} \\cdot \\frac{QUERY\\_CPU\\_LIMIT\\_PERCENT\\_PER\\_NODE}{100} = 10 vCPU \\cdot 0.7 \\cdot 0.5 = 3.5 vCPU10010vCPU⋅TOTAL_CPU_LIMIT_PERCENT_PER_NODE​⋅100QUERY_CPU_LIMIT_PERCENT_PER_NODE​=10vCPU⋅0.7⋅0.5=3.5vCPU
...
Если в очереди уже находится QUEUE_SIZE=1000QUEUE\\_SIZE = 1000QUEUE_SIZE=1000 запросов, то при отправке 1001-го запроса клиент сразу же получит в ответ ошибку, и этот запрос не будет выполнен. Пример ошибки:
...
![resource_pools](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/resources_weight.png)
...
Допустим, у нас есть узел в системе с доступными 10vCPU10 vCPU10vCPU. Установлены ограничения:

- TOTAL_CPU_LIMIT_PERCENT_PER_NODE=30TOTAL\\_CPU\\_LIMIT\\_PERCENT\\_PER\\_NODE = 30TOTAL_CPU_LIMIT_PERCENT_PER_NODE=30,
- QUERY_CPU_LIMIT_PERCENT_PER_NODE=50QUERY\\_CPU\\_LIMIT\\_PERCENT\\_PER\\_NODE = 50QUERY_CPU_LIMIT_PERCENT_PER_NODE=50.

В этом случае у пула ресурсов будет ограничение 3vCPU3 vCPU3vCPU на узел и 1.5vCPU1.5 vCPU1.5vCPU на один запрос в этом пуле (рисунок *a*). Если в системе существует 4 таких пула, и все они пытаются использовать максимальные ресурсы, это составит 12vCPU12 vCPU12vCPU, что превышает предел доступных ресурсов на узле (10vCPU10 vCPU10vCPU). В этом случае начинают действовать `RESOURCES_WEIGHT`, и каждому пулу будет выделено по 2.5vCPU2.5 vCPU2.5vCPU (рисунок *b*).

Если необходимо увеличить выделяемые ресурсы для конкретного пула, можно изменить его вес, например, на 200. Тогда этот пул получит 3vCPU3 vCPU3vCPU, а остальные пулы поделят поровну оставшиеся 7vCPU7 vCPU7vCPU, что составит 73vCPU\\frac{7}{3} vCPU37​vCPU на каждый пул (рисунок *c*).

> [!WARNING]
> Текущий алгоритм распределения ресурсов может быть изменен в будущем без поддержки обратной совместимости.

## Пул ресурсов по умолчанию {#pul-resursov-po-umolchaniyu}
...
## Управление ACL классификатора пула ресурсов {#upravlenie-acl-klassifikatora-pula-resursov}
...
## Порядок выбора классификатора пула ресурсов в случае конфликтов {#poryadok-vybora-klassifikatora-pula-resursov-v-sluchae-konfliktov}
...
## Пример приоритетного пула ресурсов {#primer-prioritetnogo-pula-resursov}
...
### Системные представления {#sistemnye-predstavleniya}
...
- [CREATE RESOURCE POOL](../yql/reference/syntax/create-resource-pool.md)
- [ALTER RESOURCE POOL](../yql/reference/syntax/alter-resource-pool.md)
- [DROP RESOURCE POOL](../yql/reference/syntax/drop-resource-pool.md)
- [CREATE RESOURCE POOL CLASSIFIER](../yql/reference/syntax/create-resource-pool-classifier.md)
- [ALTER RESOURCE POOL CLASSIFIER](../yql/reference/syntax/alter-resource-pool-classifier.md)
- [DROP RESOURCE POOL CLASSIFIER](../yql/reference/syntax/drop-resource-pool-classifier.md)
