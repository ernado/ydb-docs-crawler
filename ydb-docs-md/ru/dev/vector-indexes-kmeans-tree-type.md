---
title: "Векторный индекс вида vector_kmeans_tree"
url: "https://ydb.tech/docs/ru/dev/vector-indexes-kmeans-tree-type?version=v26.1"
doc_path: "ru/dev/vector-indexes-kmeans-tree-type"
version: "v26.1"
lang: "ru"
source_path: "ru/core/dev/vector-indexes-kmeans-tree-type.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/dev/vector-indexes-kmeans-tree-type.md"
description: "Векторные индексы — это специализированные структуры данных, которые позволяют эффективно выполнять векторный поиск в многомерных пространствах. Работа с ними п"
...
![Кластеры первого уровня](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/vector_kmeans_tree_1.png)
...
![Кластеры второго уровня](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/vector_kmeans_tree_2.png)
...
![Центроиды кластеров](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/vector_kmeans_tree_4.png)
...
![Выбор кластера первого уровня](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/vector_kmeans_tree_5.png)

Каждый кластер первого уровня разделён на кластеры второго уровня, и так далее. Алгоритм последовательно «спускается» по дереву, на каждом шаге выбирая кластер меньшего размера, пока не достигнет кластера самого нижнего уровня, который содержит не список центроидов, а непосредственно список векторов. YDB перебирает эти векторы и выбирает заданное количество ближайших к образцу. Перебирая ограниченное количество кластеров на ограниченном количестве уровней, YDB выполняет поиск со сложностью `O(1)`.

![Выбор кластера второго уровня](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/vector_kmeans_tree_6.png)

А если в кластере самого нижнего уровня меньше векторов, чем указано в запросе, то YDB вернет все найденные векторы. Чтобы повысить полноту поиска в таких сценариях, нужно воспользоваться параметром `KMeansTreeSearchTopSize`, о котором рассказано далее.

## Параметры индекса {#index-settings}

YDB позволяет настроить количество уровней векторного индекса с помощью параметра `levels` и количество кластеров на каждом уровне — с помощью параметра `clusters`. Эти параметры следует подбирать с учётом ожидаемого объёма хранимых векторов, а также характеристик кластера: сетевой задержки между узлами, пропускной способности сети и производительности центральных процессоров.

Кластеры, из которых состоит дерево векторного индекса, хранятся в распределённом хранилище. На каждом переходе вниз по дереву YDB выполняет чтение из распределённого хранилища и получает по сети бинарные данные со списком векторов. Время выполнения такой операции зависит от оборудования датацентра, сетевой нагрузки и объёма данных, передаваемых между кластерами.
...
Если центроид кластера находится ближе всего к образцу, то, как правило, векторы этого кластера также ближе к образцу, чем векторы из других кластеров. Однако при сложных топологиях данных это может не выполняться, поэтому YDB позволяет перебирать векторы более чем в одном кластере на каждом уровне дерева.

Количество кластеров, участвующих в поиске, задаётся параметром `KMeansTreeSearchTopSize`. По умолчанию он равен 1: YDB выбирает один ближайший кластер на первом уровне дерева, затем запрашивает по сети его дочерние кластеры второго уровня, снова выбирает один ближайший и так далее. Этот параметр указывается не при создании индекса, а [для поиска](../yql/reference/syntax/select/vector_index.md#kmeanstreesearchtopsize).
...
Однако такой подход означает, что после нахождения последнего кластера YDB должен выполнить `SELECT`-запрос к таблице, чтобы получить несколько сотен элементов (точное количество зависит от количества векторов в последнем кластере).
...
Индекс состоит из двух дополнительных таблиц: level table (`indexImplLevelTable`) и posting table (`indexImplPostingTable`). В таблице level table хранятся центроиды для дерева кластеров. К этой таблице YDB делает запросы при переходе с уровня на уровень во время векторного поиска. Например, если векторный индекс построен для 27 векторов, параметр `levels` установлен в значение 2, а параметр `clusters` — в значение 3, то level table будет выглядеть следующим образом:

![Level table](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/vector_kmeans_tree_7.png)

Для каждого центроида в level table также хранится информация о принадлежности к кластеру на вышестоящем уровне. YDB использует эту информацию, чтобы при переходе на нижележащий уровень запрашивать только те центроиды, которые принадлежат кластерам, выбранным на предыдущем уровне.
...
![Posting table](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/vector_kmeans_tree_8.png)
...
![Архитектура индекса с фильтрацией](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/vector_kmeans_tree_9.png)

### Структура таблицы `indexImplLevelTable` {#struktura-tablicy}
...
### Структура таблицы `indexImplPostingTable` {#struktura-tablicy1}
...
### Пример структуры таблицы `indexImplPostingTable` с покрывающим индексом {#s-pokryvayushim-indeksom}
...
### Пример структуры таблицы `indexImplPrefixTable` для индекса с фильтрацией {#dlya-indeksa-s-filtraciej}
