---
title: "Быстрый старт с YDB"
url: "https://ydb.tech/docs/ru/quickstart?version=v26.1"
doc_path: "ru/quickstart"
version: "v26.1"
lang: "ru"
source_path: "ru/core/quickstart.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/quickstart.md"
description: "В этом руководстве вы установите одноузловой локальный кластер YDB и выполните простые запросы к вашей базе данных."
revision: "95f7629e80402dd261127ed00cdc781d2b8433de"
---

# Быстрый старт с YDB

В этом руководстве вы установите одноузловой локальный [кластер YDB](concepts/glossary.md#cluster) и выполните простые запросы к вашей [базе данных](concepts/glossary.md#database).

Обычно YDB хранит данные напрямую на нескольких дисковых устройствах SSD/NVMe или HDD без использования файловой системы. Однако для простоты данное руководство эмулирует диски в оперативной памяти или с использованием файла в обычной файловой системе. Таким образом, эта конфигурация не подходит для использования в промышленном окружении и даже для проведения тестов производительности. Ознакомьтесь с [документацией для DevOps-инженеров](devops/index.md) для запуска YDB в production окружении.

## Установите и запустите YDB {#install}
...
- Docker x86_64
  ...
  >
  > Если не включена Rosetta 2, в команду запуска Docker-контейнера необходимо добавить параметр `-e YDB_USE_IN_MEMORY_PDISKS=true`. См. [Настройка Docker-контейнера YDB](reference/docker/configuration.md).
...
- Minikube
  ...
  3. Склонируйте репозиторий с [YDB Kubernetes Operator](https://github.com/ydb-platform/ydb-kubernetes-operator):
...
## Запустите первый запрос "Hello, world!" {#zapustite-pervyj-zapros-hello,-world}
...
![Стартовая страница веб-интерфейса](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/web-ui-home.png)
...
![SELECT "Hello, world!"u;](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/_assets/select-hello-world.png)
...
Второй по простоте способ выполнения SQL-запроса с использованием YDB - это [интерфейс командной строки (CLI)](reference/ydb-cli/index.md). Большинство реальных приложений же, скорее всего, будут работать с YDB через один из доступных [наборов инструментов для разработчиков программного обеспечения (SDK)](reference/ydb-sdk/index.md). Если вы чувствуете себя уверенно — можете продолжить прохождение данного руководства с помощью одного из этих методов вместо веб-интерфейса.

## Создайте свою первую таблицу {#sozdajte-svoyu-pervuyu-tablicu}
...
## Добавление тестовых данных {#dobavlenie-testovyh-dannyh}
...
## Готово! Что дальше? {#gotovo-chto-dalshe}
...
- Попробуйте создать свое первое приложение для хранения данных в YDB с использованием [одного из SDK](reference/ydb-sdk/index.md).
- Узнайте, как настроить [развертывание YDB в готовую к промышленной эксплуатации среду](devops/index.md).
- Почитайте об используемых в YDB [концепциях](concepts/index.md).
