...
description: "Версия 26.1 Версия 26.1.1.ent.3. Дата выхода: 3 августа 2026."
...
### Версия 26.1.1.ent.3 {#26-1-1-ent-3}

Дата выхода: 3 августа 2026.

Версия включает в себя все доработки, содержащиеся в сборке YDB 26.1.1.22, см. [описание изменений](changelog-server.md#26-1-1-22). Кроме того, версия включает в себя перечисленные ниже [доработки, специфичные для Корпоративной СУБД](changelog-enterprise.md#26-1-1-ent-3-extras).

#### Доработки, специфичные для Корпоративной СУБД {#26-1-1-ent-3-extras}

Перечисленные ниже изменения доступны в Корпоративной СУБД Яндекса дополнительно к соответствующей сборке YDB:

- Добавлена оптимизация, позволяющая фильтровать строки по колонкам из индекса до запроса к основной таблице, что позволяет сократить количество обращений к основной таблице при выполнении некоторых типов запросов.
- Реализован набор исправлений в индексном доступе (StreamIndexLookup), устраняющий возможность проявления редких ситуаций «зависания» выполняемых запросов и сокращающий потребление оперативной памяти при выполнении запросов.
- Некорректные представления теперь могут быть восстановлены из резервной копии. Это позволяет восстанавливать резервные копии, созданные из БД, содержащей такие представления, без дополнительных действий со стороны администратора.
- Добавлена поддержка взаимной аутентификации по сертификатам (mTLS) в [Kafka API](reference/kafka-api/index.md).
- В системные представления `.sys/top_queries_*` и `.sys/query_sessions` добавлена колонка `TraceId` с идентификатором трассировки запроса.

## Версия 25.4 {#25-4}

### Версия 25.4.1.ent.2 {#25-4-1-ent-2}

Дата выхода: 17 июня 2026.

Версия включает в себя все доработки, содержащиеся в сборке YDB 25.4.1.15, см. [описание изменений](changelog-server.md#25-4-1-15). Кроме того, включены все [дополнительные исправления](changelog-enterprise.md#25-2-1-ent-13-extras), перечисленные ниже для версии 25.2.1.ent.13.

## Версия 25.3 {#25-3}

### Версия 25.3.1.ent.3 {#25-3-1-ent-3}

Дата выхода: 11 июня 2026.

Версия включает в себя все доработки, содержащиеся в сборке YDB 25.3.1.27, см. [описание изменений](changelog-server.md#25-3-1-27). Кроме того, включены все [дополнительные исправления](changelog-enterprise.md#25-2-1-ent-13-extras), перечисленные ниже для версии 25.2.1.ent.13.
...
Версия включает в себя все доработки, содержащиеся в сборке YDB 25.2.1.26, см. [описание изменений](changelog-server.md#25-2-1-26). Кроме того, версия включает в себя ряд дополнительных доработок, портированных из актуальной версии 26.1.

#### Дополнительные исправления {#25-2-1-ent-13-extras}

Перечисленные ниже изменения были перенесены из версии 26.1 в поддерживаемые стабильные версии Корпоративной СУБД Яндекса:

- Исправлена ошибка, нарушавшая заданный в запросе порядок сортировки при обращении к системным таблицам.
- Исправлена ошибка в логике проверки целостности внутреннего состояния, которая в редких случаях могла приводить к единичному (не массовому) перезапуску узлов хранения.
- Добавлена оптимизация, позволяющая фильтровать строки по колонкам из индекса до запроса к основной таблице, что позволяет сократить количество обращений к основной таблице при выполнении некоторых типов запросов.
- Реализован набор исправлений в индексном доступе (StreamIndexLookup), устраняющий возможность проявления редких ситуаций «зависания» выполняемых запросов и сокращающий потребление оперативной памяти при выполнении запросов.
- Добавлена оптимизация, сокращающая потребление памяти при обработке запросов с операцией TopSort (`SELECT ... ORDER BY x LIMIT n`).
- Добавлена поддержка материализации индексов при резервном копировании и восстановлении.
- Сообщения об ошибках TLI (Transaction Locks Invalidated) во всех случаях снабжаются идентификатором либо путём затронутой таблицы.
- В статистики запроса, предоставляемые через системные таблицы `.sys/query_metrics_*`, добавлены метрики блокировок.
- Некорректные представления теперь могут быть восстановлены из резервной копии. Это позволяет восстанавливать резервные копии, созданные из БД, содержащей такие представления, без дополнительных действий со стороны администратора.
