...
url: "https://ydb.tech/docs/ru/reference/embedded-ui/ydb-monitoring?version=v26.1"
doc_path: "ru/reference/embedded-ui/ydb-monitoring"
version: "v26.1"
lang: "ru"
...
YDB Monitoring представлен набором web-страниц, отображающих состояние системы в разрезе по нескольким аспектам. На страницах приведены перечни компонент и их мгновенные параметры. Для многих компоненты приведен [цветовой индикатор](ydb-monitoring.md#colored_indicator) состояния перед названием.
...
![Monitoring_main_page](https://ydb.tech/docs/docs-assets/ydb-platform--ydb/rev/6a967ac39a5eac92f33ed9e017b7e5e4482fb785/ru/reference/embedded-ui/_assets/monitoring_main_page.png)
...
Далее следуют списки [тенантов](ydb-monitoring.md#tenant_list_page) и [узлов](ydb-monitoring.md#node_list_page), которые переключаются вкладками Tenants и Nodes соответственно.
...
При нажатии на путь тенанта можно перейти на [страницу тенанта](ydb-monitoring.md#tenant_page).
...
При нажатии на имя хоста, можно перейти на [страницу узла](ydb-monitoring.md#node_page).
...
На странице узла есть вкладки, Storage и Tablets содержащие [список групп хранения](ydb-monitoring.md#node_storage_page) и [список таблеток](ydb-monitoring.md#node_tablets_page) соответственно.
...
Ниже индикатора располагается список таблеток, где каждая таблетка представлена маленьким значком [цветового индикатора](ydb-monitoring.md#colored_indicator). При наведении на индикатор отображается краткая информация о таблетке:
...
- **Pools** — суммарное потребление CPU узлами тенанта в разбивке по внутренним пулам потоков (подробнее о пулах см. [страницу тенанта](ydb-monitoring.md#tenant_page));
...
- **Storage** — [список групп хранения](ydb-monitoring.md#tenant_storage_page), содержит информацию о том, какие VDisk'и на каких узлах и устройствах работают;
- **Compute** — [список узлов](ydb-monitoring.md#tenant_compute_page), содержит узлы и таблетки запущенные на них;
- **Schema** — [схема тенанта](ydb-monitoring.md#tenant_scheme), позвляет просматривать таблицы, выполнять YQL запросы, проосматривать перечень самых медленные запросов и самых нагруженных шардов;
- **Network** — [состояние сети кластера](ydb-monitoring.md#tenant_network).

### Список групп хранения в тенанте {#tenant_storage_page}

Данная страница по аналогии со [списком групп на ноде](ydb-monitoring.md#node_storage_page) показывает список групп хранения заданного тенанта. Группы хранения сгруппированы в списке по пулам хранения. Каждый пул можно развенуть в список групп.
...
В левой части страницы представлены узлы тенанта в виде квадратов [индикаторов состояний](ydb-monitoring.md#colored_indicator).
