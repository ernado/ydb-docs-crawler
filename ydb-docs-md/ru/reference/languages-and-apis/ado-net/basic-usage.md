---
title: "Основное использование ADO.NET"
url: "https://ydb.tech/docs/ru/reference/languages-and-apis/ado-net/basic-usage?version=v26.1"
doc_path: "ru/reference/languages-and-apis/ado-net/basic-usage"
version: "v26.1"
lang: "ru"
...
### С использованием строки подключения {#s-ispolzovaniem-stroki-podklyucheniya}
...
### Использование конструктора с аргументом `YdbConnectionStringBuilder` {#ispolzovanie-konstruktora-s-argumentom}
...
## Подключения {#podklyucheniya}

Подключение к YDB устанавливается через `YdbConnection`. Получение подключений осуществляется через `YdbDataSource`, это можно сделать через следующие методы:

### YdbDataSource.OpenConnectionAsync

Открывает подключение к YDB с параметрами, указанными при создании `YdbDataSource` (см. [раздел Ydb Data Source](basic-usage.md#data_source)).
...
Открывает подключение с автоматическими повторами операций (retry), учитывающее YDB Retry Policy (см. [раздел Повторные попытки](basic-usage.md#retry_policy)).
...
- Транзакции не поддерживаются. Попытка использовать транзакцию приведет к исключению (см. [раздел Транзакции](basic-usage.md#transactions)).
...
## Базовое выполнение SQL {#bazovoe-vypolnenie-sql}
...
## Другие методы выполнения {#drugie-metody-vypolneniya}
...
## Альтернативный стиль параметра с префиксом `@` {#alternativnyj-stil-parametra-s-prefiksom}
...
YDB имеет строго типизированную систему типов: столбцы и параметры имеют тип, а типы обычно не преобразуются неявным образом в другие типы. Это означает, что вам нужно подумать о том, какой тип вы будете отправлять: попытка вставить строку в целочисленный столбец (или наоборот) приведет к сбою.
...
Интерактивные транзакции и повторы (retry) — ключевая часть работы с YDB.
...
Повторные попытки выполняются в соответствии с политиками YDB (см. [раздел Повторные попытки](basic-usage.md#retry_policy)).

### Транзакции YdbConnection {#tranzakcii-ydbconnection}
...
> [!WARNING]
> Подробнее про обработку ошибок ([Transaction Locks Invalidated](../../../troubleshooting/performance/queries/transaction-lock-invalidation.md)).
...
- `BeginTransaction(TransactionMode transactionMode)`

   Параметр `Ydb.Sdk.Ado.TransactionMode` — это YDB-специфичный уровень изоляции, подробнее ознакомиться можно [здесь](../../../concepts/transactions.md).
...
### Передача настроек политики {#peredacha-nastroek-politiki}
...
### Кастомная политика повторных попыток {#kastomnaya-politika-povtornyh-popytok}
...
> [!NOTE]
> Этот раздел актуален, если вы не используете встроенные повторы провайдера (см. [раздел Повторные попытки](basic-usage.md#retry_policy)).
...
### Свойства исключения `YdbException` {#svojstva-isklyucheniya}
