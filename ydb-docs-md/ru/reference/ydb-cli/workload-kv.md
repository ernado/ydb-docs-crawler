---
title: "Key-Value нагрузка"
url: "https://ydb.tech/docs/ru/reference/ydb-cli/workload-kv?version=v26.1"
doc_path: "ru/reference/ydb-cli/workload-kv"
version: "v26.1"
lang: "ru"
...
- [upsert](workload-kv.md#upsert-kv) — при помощи операции `UPSERT` вставляет в заранее созданную командой init таблицу строки, которые представляют из себя кортежи `(key1, key2, ... keyK, value1, value2, ... valueN)`, числа K и N задаются в параметрах.
- [insert](workload-kv.md#insert-kv) — функционал такой же, как у нагрузки upsert, но для вставки используется операция `INSERT`.
- [select](workload-kv.md#select-kv) — читает данные при помощи операции `SELECT * WHERE key = $key`. Запрос всегда затрагивает все колонки таблицы, но не всегда является точечным, количеством вариаций primary ключа можно управлять с помощью параметров.
- [read-rows](workload-kv.md#read-rows-kv) - читает данные при помощи операции ReadRows, выполняющей более быстрые чтения по ключу, чем `SELECT`. Запрос всегда затрагивает все колонки таблицы, но не всегда является точечным, количеством вариаций primary ключа можно управлять с помощью параметров.
- [mixed](workload-kv.md#mixed-kv) - одновременно пишет и читает данные, дополнительно проверяя что все записанные данные успешно читаются.
...
- `init options` — [параметры инициализации](workload-kv.md#init-options).
...
| `--max-first-key` | Максимальное значение primary ключа таблицы. Значение по умолчанию: 264—12^{64} — 1264—1. |
...
- `workload type` — [виды нагрузки](workload-kv.md#workload-types).
- `global workload options` — [общие параметры для всех видов нагрузки](workload-kv.md#global-workload-options).
...
- `global workload options` - [общие параметры для всех видов нагрузки](workload-kv.md#global-workload-options).
- `specific workload options` - [параметры конкретного вида нагрузки](workload-kv.md#upsert-options).
...
- `global workload options` - [общие параметры для всех видов нагрузки](workload-kv.md#global-workload-options).
- `specific workload options` - [параметры конкретного вида нагрузки](workload-kv.md#insert-options).
...
- `global workload options` - [общие параметры для всех видов нагрузки](workload-kv.md#global-workload-options).
- `specific workload options` - [параметры конкретного вида нагрузки](workload-kv.md#mixed-options).
...
| `--do-select` | Включение/выключение чтений при помощи запроса [select](workload-kv.md#select-kv). Возможные значения: 0 или 1. Значение по умолчанию: 1. |
| `--do-read-rows` | Включение/выключение чтений при помощи запроса [read-rows](workload-kv.md#read-rows-kv). Возможные значения: 0 или 1. Значение по умолчанию: 1. |
