---
title: "Настройка ydbops"
url: "https://ydb.tech/docs/ru/reference/ydbops/configuration?version=v26.1"
doc_path: "ru/reference/ydbops/configuration"
version: "v26.1"
lang: "ru"
...
- [Файл конфигурации](configuration.md#config-file)
- [Переменные окружения](configuration.md#environment-variables)

## Файл конфигурации {#config-file}

Файл конфигурации для `ydbops` — это файл в формате YAML, содержащий несколько профилей. Профили для `ydbops` работают так же, как и профили в [YDB CLI](../ydb-cli/profile/index.md), однако они не взаимозаменяемы, поскольку могут содержать параметры, которые актуальны для `ydbops`, но не имеют смысла для YDB CLI, и наоборот.

Расположение файла конфигурации по умолчанию следует той же конвенции, что и YDB CLI, и находится в той же папке в подкаталоге `ydbops`. Для сравнения:

- файл конфигурации по умолчанию для YDB CLI : `$HOME/.ydb/config/config.yaml`
...
### Команды управления профилями {#komandy-upravleniya-profilyami}

В настоящее время `ydbops` не поддерживает создание, изменение и активацию профилей через команды CLI [так, как это делает YDB CLI](../ydb-cli/profile/index.md#commands).

Файл конфигурации необходимо создать и редактировать вручную.

### Максимально полный файл конфигурации {#maksimalno-polnyj-fajl-konfiguracii}
...
Кроме того, можно указать несколько переменных окружения вместо передачи аргументов командной строки или использования [файлов конфигурации](configuration.md#config-file).
...
- [Обзор утилиты ydbops](index.md)
- [Установка ydbops](install.md)
- [Выполнение перезапуска кластера YDB с помощью ydbops](rolling-restart-scenario.md)
