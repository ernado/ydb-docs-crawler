---
title: "Аутентификация в SDK"
url: "https://ydb.tech/docs/ru/reference/ydb-sdk/auth?version=v26.1"
doc_path: "ru/reference/ydb-sdk/auth"
version: "v26.1"
lang: "ru"
...
Как описано в статье о [подключении к серверу YDB](../../concepts/connect.md), клиент с каждым запросом должен отправить [аутентификационный токен](../../security/authentication.md). Аутентификационный токен проверяется сервером и, в случае успешной аутентификации, запрос авторизуется и выполняется, иначе возвращается ошибка `Unauthenticated`.
...
5. Иначе, если задано значение переменной окружения `YDB_OAUTH2_KEY_FILE`, то используется режим аутентификации **OAuth 2.0 token exchange**, а настройки загружаются из [JSON файла](auth.md#oauth2-key-file-format), указанного в данной переменной.
...
## Особенности YDB Python SDK v2 (устаревшая версия) {#osobennosti-ydb-python-sdk-v2-ustarevshaya-versiya}
...
- В случае, если при инициализации драйвера не передан никакой объект, отвечающий за генерацию токенов, то применяется [общий порядок](auth.md#env) чтения значений переменных окружения.
