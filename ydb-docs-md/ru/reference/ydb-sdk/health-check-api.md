...
url: "https://ydb.tech/docs/ru/reference/ydb-sdk/health-check-api?version=v26.1"
doc_path: "ru/reference/ydb-sdk/health-check-api"
version: "v26.1"
lang: "ru"
source_path: "ru/core/reference/ydb-sdk/health-check-api.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/reference/ydb-sdk/health-check-api.md"
description: "YDB имеет встроенную систему самодиагностики, с помощью которой можно получить краткий отчет о состоянии базы данных и информацию об имеющихся проблемах."
...
`SelfCheck` возвращает информацию в форме [списка проблем](health-check-api.md#example-emergency), каждая из которых может выглядеть так:
...
| Поле | Тип | Описание |
| --- | --- | --- |
| `ReturnVerboseStatus` | `bool` | Если задано, ответ также будет содержать сводку общего состояния базы данных в поле `database_status` ([Пример](health-check-api.md#example-verbose)). По умолчанию — `false`. |
| `MinimumStatus` | [EStatusFlag](health-check-api.md#issue-status) | Каждая проблема содержит поле `status`. Если `minimum_status` определён, проблемы с менее серьёзным статусом будут отброшены. По умолчанию будут перечислены все проблемы. |
...
Полную структуру ответа можно посмотреть в файле [ydb_monitoring.proto](https://github.com/ydb-platform/ydb/blob/main/ydb/public/api/protos/ydb_monitoring.proto) в Git репозитории YDB.  
...
| Поле | Описание |
| --- | --- |
| `self_check_result` | Перечисляемое поле, содержащее [результат проверки базы данных](health-check-api.md#selfcheck-result). |
...
| `issue_log.status` | Перечисляемое поле, содержащее [статус проблемы](health-check-api.md#issue-status). |
...
| `issue_log.type` | Категория проблемы. Каждый тип находится на определённом уровне и связан с другими через [жёсткую иерархию](health-check-api.md#issues-hierarchy) (как показано на изображении выше). |
| `issue_log.level` | [Глубина вложенности](health-check-api.md#issues-hierarchy) проблемы. |
| `database_status` | Если в настройках задан параметр `verbose`, то поле `database_status` будет заполнено. Оно предоставляет сводку общего состояния базы данных и используется для быстрой оценки состояния базы данных и выявления серьёзных проблем на высоком уровне. [Пример](health-check-api.md#example-verbose). Полную структуру ответа можно посмотреть в файле [ydb_monitoring.proto](https://github.com/ydb-platform/ydb/blob/main/ydb/public/api/protos/ydb_monitoring.proto) в Git репозитории YDB. |
...
![cards_hierarchy](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/reference/ydb-sdk/_assets/hc_cards_hierarchy.png)
...
![issues_hierarchy](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/reference/ydb-sdk/_assets/hc_types_hierarchy.png)
...
#### Database has multiple issues, Database has compute issues, Database has storage issues {#database-has-multiple-issues,-database-has-compute-issues,-database-has-storage-issues}
...
#### Storage degraded, Storage has no redundancy, Storage failed {#storage-degraded,-storage-has-no-redundancy,-storage-failed}
...
#### Storage usage over 75%, Storage usage over 85%, Storage usage over 90% {#storage-usage-over-75percent,-storage-usage-over-85percent,-storage-usage-over-90percent}

**Описание:** Необходимо увеличить дисковое пространство.

### STORAGE_POOL {#storage_pool}

#### Pool degraded, Pool has no redundancy, Pool failed {#pool-degraded,-pool-has-no-redundancy,-pool-failed}
...
#### Available size is less than 12%, Available size is less than 9%, Available size is less than 6% {#available-size-is-less-than-12percent,-available-size-is-less-than-9percent,-available-size-is-less-than-6percent}
...
#### Paths quota usage is over than 90%, Paths quota usage is over than 99%, Paths quota exhausted, Shards quota usage is over than 90%, Shards quota usage is over than 99%, Shards quota exhausted {#paths-quota-usage-is-over-than-90percent,-paths-quota-usage-is-over-than-99percent,-paths-quota-exhausted,-shards-quota-usage-is-over-than-90percent,-shards-quota-usage-is-over-than-99percent,-shards-quota-exhausted}
...
### SYSTEM_TABLET {#system_tablet}

#### System tablet is unresponsive, System tablet response time over 1000ms, System tablet response time over 5000ms {#system-tablet-is-unresponsive,-system-tablet-response-time-over-1000ms,-system-tablet-response-time-over-5000ms}
...
#### Tablets are dead, Followers are dead {#tablets-are-dead,-followers-are-dead}
...
#### LoadAverage above 100% {#loadaverage-above-100percent}
...
#### Pool usage is over than 90%, Pool usage is over than 95%, Pool usage is over than 99% {#pool-usage-is-over-than-90percent,-pool-usage-is-over-than-95percent,-pool-usage-is-over-than-99percent}
...
### NODES_TIME_DIFFERENCE {#nodes_time_difference}

#### Node is ... ms behind peer \[id\], Node is ... ms ahead of peer \[id\] {#node-is-ms-behind-peer-[id],-node-is-ms-ahead-of-peer-[id]}
