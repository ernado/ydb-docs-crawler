---
title: "log_config"
url: "https://ydb.tech/docs/ru/reference/configuration/log_config?version=v26.1"
doc_path: "ru/reference/configuration/log_config"
version: "v26.1"
lang: "ru"
...
> [!NOTE]
> Этот документ описывает конфигурацию прикладного и системного логирования. Для информации о логировании для целей аудита и обеспечения безопасности, см. [Аудитный лог](../../security/audit-log.md).

## Обзор {#obzor}

Логирование является критически важной частью системы [наблюдаемости](../observability/index.md) YDB. Секция `log_config` позволяет настраивать различные аспекты логирования, включая:
...
## Способы вывода логов {#sposoby-vyvoda-logov}
...
| `entry` | array | \[\] | Конфигурация уровня логирования и/или семплирования для конкретных компонентов YDB, cм. [Объекты Entry](log_config.md#entry-objects) ниже. |
| `uaclient_config` | object | — | Конфигурация для клиента Unified Agent, см. [Объект UAClientConfig](log_config.md#uaclient-config) ниже. |
...
| `level` | uint32 | [Уровень логирования](log_config.md#log-levels) для этого компонента. |
...
### Базовая конфигурация {#bazovaya-konfiguraciya}
...
### Конфигурация с выводом в файл {#konfiguraciya-s-vyvodom-v-fajl}
...
### Конфигурация с выводом в syslog {#konfiguraciya-s-vyvodom-v-syslog}
...
### Настройка уровней логирования для отдельных компонентов {#nastrojka-urovnej-logirovaniya-dlya-otdelnyh-komponentov}
...
После включения сервер записывает [логи](../../devops/observability/logging.md) при каждом конфликте блокировок, включая идентификаторы и тексты запросов-жертв и нарушителей. Подробнее о формате записей, корреляции событий и утилите `find_tli_chain` см. в разделе [Логирование TLI](../../troubleshooting/performance/queries/tli-logging.md).

### Конфигурация семплирования {#konfiguraciya-semplirovaniya}
...
### Конфигурация формата JSON {#konfiguraciya-formata-json}
...
- Уровни логирования указываются в конфигурации как числовые значения, а не строки. Используйте [таблицу выше](log_config.md#log-levels) для сопоставления между числовыми значениями и их значениями.
...
- [Справка по наблюдаемости YDB](../observability/index.md)
- [Справка по метрикам](../observability/metrics/index.md)
- [Трассировка в YDB](../observability/tracing/setup.md)
- [Аудитный лог](../../security/audit-log.md)
