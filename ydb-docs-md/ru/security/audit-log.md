---
title: "Аудитный лог"
url: "https://ydb.tech/docs/ru/security/audit-log?version=v26.1"
doc_path: "ru/security/audit-log"
version: "v26.1"
lang: "ru"
source_path: "ru/core/security/audit-log.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/security/audit-log.md"
description: "Аудитный лог — это поток, который содержит информацию обо всех операциях (успешных или не успешных) над объектами YDB:"
revision: "e9f541853a7760e5c0d0babc071d86df7f523cf5"
---

# Аудитный лог

*Аудитный лог* — это поток, который содержит информацию обо всех операциях (успешных или не успешных) над объектами YDB:
...
- в файл на каждом узле кластера YDB;
...
Направление аудитного лога в стандартный вывод ошибок `stderr` рекомендуется для тестовых инсталляций. Дальнейшая обработка данных потока определяется настройками [логирования](../devops/observability/logging.md) кластера YDB.

## События аудитного лога {#events}

Информация о каждой операции записывается в аудитный лог в виде отдельного события. Каждое событие содержит набор атрибутов. Одни атрибуты являются общими для любых событий, другие определяются компонентом YDB, в котором произошло событие.
...
| `status` | Статус завершения операции.  <br>Возможные значения:<br>- `SUCCESS` — операция завершена успешно;<br>- `ERROR` — операция завершена с ошибкой;<br>- `IN-PROCESS` — операция выполняется.<br>Обязательный. |
| `reason` | Сообщение об ошибке.  <br>Необязательный. |
| `component` | Имя компонента YDB — источника события (например, `schemeshard`).  <br>Необязательный. |
...
| `detailed_status` | Статус, который передает компонент YDB (например `StatusAccepted`, `StatusInvalidParameter`, `StatusNameConflict`).  <br>Необязательный. |
...
Отправка событий в поток аудитного лога включается целиком для кластера YDB. Для включения необходимо в [конфигурацию кластера](../reference/configuration/index.md) добавить секцию `audit_config`, указать в ней одно из назначений для потока (`file_backend`, `unified_agent_backend`, `stderr_backend`) или их комбинацию:
...
| Ключ | Описание |
| --- | --- |
| `file_backend` | Сохранять аудитный лог в файл на каждом узле кластера.Необязательный. |
| `format` | Формат аудитного лога. Значение по умолчанию: `JSON`.  <br>Возможные значения:<br>- `JSON` — [JSON](https://ru.wikipedia.org/wiki/JSON) в сериализованном виде;<br>- `TXT` — текст.<br>Необязательный. |
...
| `unified_agent_backend` | Направить аудитный лог в Unified Agent. Необходимо также описать секцию `uaclient_config` в [конфигурации кластера](../reference/configuration/index.md).Необязательный. |
...
| `stderr_backend` | Направить аудитный лог в стандартный вывод ошибок `stderr`.Необязательный. |
