...
url: "https://ydb.tech/docs/ru/security/authentication?version=v26.1"
doc_path: "ru/security/authentication"
version: "v26.1"
lang: "ru"
source_path: "ru/core/security/authentication.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/security/authentication.md"
description: "После успешной установки сетевого соединения сервер принимает к обработке запросы от клиента, в которых передается аутентификационная информация в виде аутентиф"
...
> [!NOTE]
> Под клиентом аутентификации понимается пользователь, проходящий процедуру проверки подлинности при доступе к YDB. Примерами клиентов являются приложения, использующие [SDK](../reference/ydb-sdk/index.md) или [CLI](../reference/ydb-cli/index.md).

Поддерживаются следующие виды аутентификации:

- [Анонимная](authentication.md#anonymous) аутентификация.
- Аутентификация по [логину и паролю](authentication.md#static-credentials).
- Аутентификация с использованием [LDAP-каталога](authentication.md#ldap).
- [Аутентификация с использованием стороннего IAM-провайдера](authentication.md#iam), например [Yandex Identity and Access Management](https://yandex.cloud/ru/docs/iam/).

## Анонимная аутентификация {#anonymous}

По умолчанию, YDB допускает выполнение запросов без указания аутентификационных данных, таких как имя пользователя или [токен](../concepts/glossary.md#auth-token). Проверка прав доступа ([авторизация](authorization.md)) при этом также не проводится.
...
За выключение режима анонимной аутентификации отвечает флаг `enforce_user_token_requirement` в [настройках режима аутентификации](../reference/configuration/auth_config.md#security-auth) YDB.
...
Аутентификация по логину и паролю через сервер YDB доступна только для [локальных пользователей](../concepts/glossary.md#access-user). В аутентификации внешних пользователей участвуют сервера внешних систем.
...
Данный вид доступа подразумевает наличие у каждого пользователя базы данных логина и пароля.  
 ...
 Допускается установление различных [критериев](authentication.md#password-complexity) на сложность пароля.
...
Об управлении ролями и пользователями читайте в [Авторизация](authorization.md).

### Сложность пароля {#password-complexity}

YDB позволяет настраивать требования к сложности паролей. Если пароль, предоставляемый через команды `CREATE USER` или `ALTER USER`, не соответствует критериям сложности, выполнение команды завершится ошибкой.  
...
### Принудительная блокировка/разблокировка пользователя {#prinuditelnaya-blokirovka/razblokirovka-polzovatelya}
...
### Защита от перебора пароля {#zashita-ot-perebora-parolya}

YDB предоставляет защиту от перебора пароля пользователем. Пользователь будет считаться заблокированным, если превысит число попыток ввода неправильного пароля. По истечении указанного времени он снова получит возможность аутентифицироваться.

> [!NOTE]
> Данный механизм применим только для пользователей, которые обслуживаются самой YDB, для, так называемых, встроенных пользователей. Пользователи, обслуживаемые внешними источниками аутентификации, такими как LDAP серверы, не подпадают под действие механизма защиты от перебора пароля.
...
На данный момент YDB поддерживает только один способ LDAP-аутентификации пользователей — *search+bind*: после получения логина и пароля выполняется служебный *bind* от имени [сервисного аккаунта](authentication.md#ldap-service-account-auth) (логин и пароль в `bind_dn` / `bind_password` либо сертификат и SASL EXTERNAL — см. [конфигурацию](../reference/configuration/auth_config.md#ldap-auth-config)), затем поиск записи пользователя и второй *bind* уже от его имени.
...
Учетные данные сервисного аккаунта для подключения к LDAP указываются в настройках конфигурации: используйте параметры `bind_dn` и `bind_password`, либо настройте mTLS (подробнее см. в разделе [Аутентификация сервисного аккаунта](authentication.md#ldap-service-account-auth)).

Далее процесс аутентификации проходит по следующей схеме:

1. YDB подключается к LDAP от имени сервисного аккаунта.
...
3. Если пользователь найден, YDB выполняет повторную операцию bind — уже от имени найденного пользователя, с использованием его пароля.
4. Итоговый результат — успешная или неуспешная аутентификация — определяется результатом второго bind (от лица пользователя).

Таким образом, YDB не хранит пароли пользователей и полностью полагается на механизм аутентификации LDAP.

В результате успешной проверки логина и пароля пользователя в LDAP каталоге возвращается [аутентификационный токен](../concepts/glossary.md#auth-token) YDB. Далее этот токен используется вместо логина и пароля. Оперирование токеном ускоряет процесс аутентификации и повышает безопасность.
...
### Проверка токена {#proverka-tokena}
...
> [!WARNING]
> Чтобы различать, что введенный логин должен быть логином пользователя из LDAP каталога, а не логином локального пользователя YDB, к нему нужно добавлять суффикс `@ldap`.
>
> Ниже приведены примеры аутентификации пользователя `user1` используя [YDB CLI](../reference/ydb-cli/index.md):
>
...
- Автоматически. Используется схема соединения [`ldaps`](authentication.md#ldaps)
- Использование расширения LDAP протокола [`StartTls`](authentication.md#starttls)
...
- **OAuth 2.0 token exchange** - клиент (SDK или CLI) обменивает токен другого типа на access token по [протоколу обмена токенов OAuth 2.0](https://www.rfc-editor.org/rfc/rfc8693), который затем передаётся в запросы YDB API.
