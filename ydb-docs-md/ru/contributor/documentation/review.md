---
title: "Процесс ревью документации YDB"
url: "https://ydb.tech/docs/ru/contributor/documentation/review?version=v26.1"
doc_path: "ru/contributor/documentation/review"
version: "v26.1"
lang: "ru"
...
Развивая общий обзор из [Участие в разработке документации YDB](index.md), эта статья подробнее рассматривает, что происходит на этапе ревью pull-запроса с документацией.

## Роли {#roli}

- **Автор** — человек, предлагающий изменения в документации.
- **Основной ревьюер** — человек, тщательно проверяющий предложенные изменения согласно [контрольному списку](review.md#checklist).
...
4. **Основной ревьюер** назначается автоматически или выбирает pull-запрос из [входящего списка](https://github.com/ydb-platform/ydb/pulls?q=is%3Aopen+is%3Apr+label%3Adocumentation+draft%3Afalse+no%3Aassignee) с помощью кнопки «assign yourself» в правом сайдбаре, а затем предоставляет начальную обратную связь и набор предложений по улучшению.
   ...
   <details>
   <summary>Временное исключение себя из автоматического назначения, если вы являетесь основным ревьюером</summary>

   ![Исключение себя из автоматического назначения основных ревьюеров](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/contributor/documentation/_assets/excluding-primary-reviewer.jpeg)

   </details>

5. **Автор** и **основной ревьюер** совместно итеративно работают, пока предложенные изменения не пройдут [контрольный список](review.md#checklist). **Основной ревьюер** предоставляет отзывы через комментарии к pull-запросу, а **автор** их учитывает. Ожидаемое время ответа для каждой итерации ревью составляет два рабочих дня, до нескольких недель в случае форс-мажора. Также **основной ревьюер** периодически пересматривает назначенные на себя pull-запросы и уточняет с авторами статус, если несколько рабочих дней в них ничего не происходит (в комментариях на GitHub, плюс по возможности в личной переписке).
...
![Пример включения auto-merge и отклонения устаревшего отзыва](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/contributor/documentation/_assets/automerge-and-dismiss.png)

- Отклоняет свой устаревший отзыв c комментарием [«lgtm»](https://en.wiktionary.org/wiki/LGTM).

![Пример отклонения с комментарием «lgtm»](https://raw.githubusercontent.com/ydb-platform/ydb/main/ydb/docs/ru/core/contributor/documentation/_assets/lgtm.png)
...
10. Документация YDB многоязычная, и от **авторов** ожидается предоставление синхронизированных изменений для всех поддерживаемых языков (в настоящее время английский и русский), если это применимо. Если **автор** не знает все необходимые языки, допустимо использование [LLM](https://ru.wikipedia.org/wiki/%D0%91%D0%BE%D0%BB%D1%8C%D1%88%D0%B0%D1%8F_%D1%8F%D0%B7%D1%8B%D0%BA%D0%BE%D0%B2%D0%B0%D1%8F_%D0%BC%D0%BE%D0%B4%D0%B5%D0%BB%D1%8C) или машинного перевода. На каком этапе делать перевод зависит от сложности:
...
### Если PR добавляет новый контент в ветку main {#esli-pr-dobavlyaet-novyj-kontent-v-vetku-main}
...
- \[ \] Терминология консистентна. Первое упоминание каждого термина, используемого в статье, является ссылкой на его объяснение в [Глоссарий YDB](../../concepts/glossary.md) или известном доверенном источнике, таком как Википедия.
...
> [!TIP]
> Этот контрольный список является сжатой версией [Руководство по стилю документации YDB](style-guide.md) и служит напоминанием при каждом ревью. Его можно копировать и вставлять в описание pull-запроса и отмечать пункты по мере продвижения процесса ревью. Обращайтесь к полному руководству по стилю для изначального понимания этих пунктов и дополнительных деталей.

### Если PR является обратным переносом в стабильную ветку {#esli-pr-yavlyaetsya-obratnym-perenosom-v-stabilnuyu-vetku}
...
## Чем не является ревью документации {#chem-ne-yavlyaetsya-revyu-dokumentacii}
...
### Ревью технического дизайна {#revyu-tehnicheskogo-dizajna}
