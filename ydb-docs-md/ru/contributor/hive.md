---
title: "Hive"
url: "https://ydb.tech/docs/ru/contributor/hive?version=v26.1"
doc_path: "ru/contributor/hive"
version: "v26.1"
lang: "ru"
source_path: "ru/core/contributor/hive.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/contributor/hive.md"
description: "Hive — таблетка, отвечающая за управление другими таблетками, а именно за выбор узлов базы для их запуска, принятие решений о необходимости перебалансировки таб"
...
Логика запуска таблеток подробно описана в статье [Запуск таблеток](hive-booting.md).
...
Scatter=MaxUsage−MinUsageMaxUsage\\mathrm{Scatter} = \\frac{\\mathrm{MaxUsage} - \\mathrm{MinUsage}}{\\mathrm{MaxUsage}} Scatter=MaxUsageMaxUsage−MinUsage​

MaxUsage\\mathrm{MaxUsage}MaxUsage и MinUsage\\mathrm{MinUsage}MinUsage — максимум и минимум потребления данного ресурса среди всех узлов, соответственно. Для нормировки потребления на каждом узле используется число доступных ресурсов на узле, которое может различаться между узлами. При низких нагрузках эта величина может сильно колебаться. Чтобы этого избежать, при вычислении Scatter\\mathrm{Scatter}Scatter считается, что потребление ресурса не может быть ниже 30%. Если ScatterScatterScatter превышает порог, запускается балансировка.

Максимальное из значений ScatterScatterScatter по разным ресурсам доступно в виде [сенсора](../devops/observability/monitoring.md) `Hive/MAX(BalanceScatter)` в подгруппе `tablets`.
