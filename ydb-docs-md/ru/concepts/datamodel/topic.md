---
title: "Топик"
url: "https://ydb.tech/docs/ru/concepts/datamodel/topic?version=v26.1"
doc_path: "ru/concepts/datamodel/topic"
version: "v26.1"
lang: "ru"
...
Приложение-писатель записывает сообщения в топик. Приложения-читатели независимы друг от друга, они получают, "считывают", сообщения из топика в том порядке, в котором они были записаны. С помощью топика реализуется архитектурный шаблон [издатель-подписчик](https://ru.wikipedia.org/wiki/%D0%98%D0%B7%D0%B4%D0%B0%D1%82%D0%B5%D0%BB%D1%8C_%E2%80%94_%D0%BF%D0%BE%D0%B4%D0%BF%D0%B8%D1%81%D1%87%D0%B8%D0%BA).
...
- Гарантиями последовательной обработки сообщений ([FIFO](https://en.wikipedia.org/wiki/Message_queue)), опубликованных с одинаковым [идентификатором источника](topic.md#producer-id).
...
- **Неактивные.** Из них можно только читать. Неактивные партиции появляются после разделения партиции при включенном [автопартиционировании](topic.md#autopartitioning). Неактивные партиции удаляются автоматически, когда все сообщения из такой партиции удалены по истечении времени хранения.
...
3. Таким образом, гарантии exactly-once записи и порядка чтения продолжают выполняться для конкретного [идентификатора источника (producer-id)](topic.md#producer-id).

### Режимы автопартиционирования {#autopartitioning_modes}

Для любого топика возможны следующие режимы автопартиционирования.

#### Выключено (DISABLED) {#vyklyucheno-disabled}
...
### Когда важен порядок обработки сообщений {#kogda-vazhen-poryadok-obrabotki-soobshenij}
...
Для каждого топика определено время хранения сообщений. После истечения времени хранения сообщения автоматически удаляются.  
 Исключение составляют данные, которые ещё не были подтверждены ["важным"](topic.md#important-consumer) читателем — они будут храниться до тех пор, пока читатель их не обработает.  
 При наличии читателя с явно указанным [временем доступности](topic.md#availability-period-consumer), время хранения необработанных читателем сообщений увеличивается до указанного значения.
...
Позиция чтения — это сохраненное [смещение](topic.md#offset) читателя по каждой партиции топика. Позиция чтения сохраняется читателем после отправки подтверждения прочитанных данных. При установке новой сессии чтения сообщения поступают читателю начиная с сохраненной позиции чтения. Это позволяет пользователям не хранить позицию чтения на своей стороне.
...
Эта опция позволяет продлить время хранения сообщений в топике с [времени хранения](topic.md#retention-time) вплоть до указанного времени доступности, если читатель не подтверждает обработку.  
 В отличие от признака ["важного"](topic.md#important-consumer) читателя, параметр ограничивает максимальный возраст сообщений, которые будут храниться в топике.  
 Если опция не задана или все читатели подтверждают обработку без большой задержки (меньше [времени хранения](topic.md#retention-time) топика), то данные будут удалены по обычным правилам.
...
YDB поддерживает работу с топиками в рамках [транзакций](../transactions.md).
