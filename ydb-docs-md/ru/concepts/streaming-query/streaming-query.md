...
url: "https://ydb.tech/docs/ru/concepts/streaming-query/streaming-query?version=v26.1"
doc_path: "ru/concepts/streaming-query/streaming-query"
version: "v26.1"
lang: "ru"
...
YDB реализует потоковую обработку как часть единой платформы для работы с данными. В дополнение к типовым сценариям использования потоковых запросов, интеграция в общую платформу YDB позволяет получать данные из [топиков](../datamodel/topic.md) YDB, обрабатывать потоки изменений из [строковых таблиц](../datamodel/table.md#row-oriented-tables) через [CDC](../cdc.md), а результаты обработки записывать в выходные топики или напрямую в таблицы. Это позволяет строить конвейеры обработки данных внутри YDB с минимальными временными задержками.
...
- **Внешние приложения** — через [YDB SDK](../../reference/ydb-sdk/index.md) или [Kafka API](../../reference/kafka-api/index.md). Например, сервис отправляет события телеметрии или логи в топик YDB, а потоковый запрос обрабатывает их.
- **CDC (Change Data Capture)** — потоки изменений из таблиц, реализуемые через встроенные [топики](../datamodel/topic.md). Позволяют реагировать на вставки, обновления и удаления записей в реальном времени. Подробнее: [Change Data Capture (CDC)](../cdc.md).
...
Подробное описание гарантий, аномалий и способов их минимизации — в разделе [Гарантии доставки данных](../../dev/streaming-query/guarantees.md).
...
> - Не поддерживается обогащение из таблиц YDB — потоковые запросы могут использовать только [внешние таблицы S3](../query_execution/federated_query/s3/external_table.md) для обогащения данными (поддержка обогащения из таблиц YDB планируется к реализации в версии 26.1).
...
- Создайте [внешний источник данных](../datamodel/external_data_source.md), указывающий на ту же или другую базу данных YDB, и обращайтесь к топикам через них.
...
- [Гарантии доставки данных](../../dev/streaming-query/guarantees.md) — гарантии, аномалии при оконной агрегации и рекомендации;
- [Быстрый старт: чтение и запись в топики](../../recipes/streaming_queries/topics.md) — пошаговое руководство;
- [Форматы данных при чтении/записи из топиков](../../dev/streaming-query/streaming-query-formats.md) — форматы данных.
