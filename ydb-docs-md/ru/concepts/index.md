---
title: "Концепции YDB"
url: "https://ydb.tech/docs/ru/concepts/?version=v26.1"
doc_path: "ru/concepts/"
version: "v26.1"
lang: "ru"
source_path: "ru/core/concepts/index.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/concepts/index.md"
description: "Данный раздел документации охватывает фундаментальные концепции и архитектурные принципы YDB. Эта информация поможет вам лучше понимать, что происходит при изуч"
revision: "6cf79b202d43efced4776d730126c3952ec44c6d"
---

# Концепции YDB

Данный раздел документации охватывает фундаментальные концепции и архитектурные принципы YDB. Эта информация поможет вам лучше понимать, что происходит при изучении более практического контента, который можно найти в [других разделах, предназначенных для конкретных ролей](index.md#see-also).

Следующие темы обеспечивают всестороннее освещение основной функциональности YDB, от высокоуровневой архитектуры и модели данных до продвинутых возможностей, таких как транзакции, индексирование и федеративные запросы.

- [Глоссарий](glossary.md)
- [Аналитические возможности](analytics/index.md)
- [Потоковая обработка](streaming-query/index.md)
- [Архитектура](architecture.md)
- [Соединение с БД](connect.md)
- [Схемные объекты](datamodel/index.md)
- [Топология кластера](topology.md)
- [Выполнение запросов](query_execution/index.md)
- [Транзакции](transactions.md)
- [Change Data Capture (CDC)](cdc.md)
- [Time to Live (TTL) и вытеснение данных во внешнее хранилище](ttl.md)
- [Ограничения базы данных](limits-ydb.md)
- [Асинхронная репликация](async-replication.md)
- [Режим bridge](bridge.md)
- [Трансфер данных](transfer.md)

## См. также {#see-also}

- [YDB для DevOps-инженеров](../devops/index.md)
- [YDB для разработчиков приложений](../dev/index.md)
- [YDB для инженеров по безопасности](../security/index.md)
