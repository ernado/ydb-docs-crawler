...
url: "https://ydb.tech/docs/ru/concepts/cdc?version=v26.1"
doc_path: "ru/concepts/cdc"
version: "v26.1"
lang: "ru"
source_path: "ru/core/concepts/cdc.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/concepts/cdc.md"
...
> [!WARNING]
> Поддерживается только для [строковых](datamodel/table.md#row-oriented-tables) таблиц. Поддержка функциональности для [колоночных](datamodel/table.md#column-oriented-tables) таблиц находится в разработке.
...
Барьеры — служебные записи без данных об изменении или удалении с [виртуальными метками времени](cdc.md#virtual-timestamps), которые появляются в каждой партиции топика с заданной периодичностью. Барьер — это гарантия, что любое изменение с виртуальной меткой времени меньшей, чем у барьера, было записано в данную партицию топика.
...
> [!NOTE]
> Запись с исходным значением строки будет помечена как запись об [обновлении](cdc.md#restrictions). При использовании [виртуальных меток времени](cdc.md#virtual-timestamps) записи маркируются меткой времени снапшота.
...
> [!WARNING]
> На время первоначального сканирования в таблице приостанавливаются процессы [автоматического партиционирования](datamodel/table.md#partitioning_row_table) и в топик не выгружаются [барьеры](cdc.md#barriers).
...
- `ts` — [виртуальная метка времени](cdc.md#virtual-timestamps). Присутствует, если включена настройка `VIRTUAL_TIMESTAMPS`. Содержит значение глобального времени координатора (`step`) и уникальный идентификатор транзакции (`txId`).
...
- `op` — операция, которая была произведена над строкой в таблице:
  ...
  - `u` — обновление.
  - `d` — удаление.
  - `r` — чтение из [снапшота](cdc.md#initial-scan).
...
- `source` — метаданные записи.
  ...
  - `step` — глобальное время координатора. Компонент [виртуальных меток времени](cdc.md#virtual-timestamps).
  - `txId` — уникальный идентификатор транзакции. Компонент [виртуальных меток времени](cdc.md#virtual-timestamps).
...
Для получения параметров топика можно воспользоваться [SDK](../reference/ydb-sdk/topic.md#describe-topic) или [YDB CLI](../reference/ydb-cli/commands/scheme-describe.md), передав в аргументах путь до потока изменений, который формируется следующим образом:
