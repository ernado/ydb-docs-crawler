---
title: "ru/yql/reference/syntax/select/group-by"
url: "https://ydb.tech/docs/ru/yql/reference/syntax/select/group-by?version=v26.1"
doc_path: "ru/yql/reference/syntax/select/group-by"
version: "v26.1"
lang: "ru"
source_path: "ru/core/yql/reference/syntax/select/group-by.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/yql/reference/syntax/select/group-by.md"
description: "GROUP BY."
revision: "e9f541853a7760e5c0d0babc071d86df7f523cf5"
---

# ru/yql/reference/syntax/select/group-by
...
Имеется возможность выполнять группировку по результату вычисления произвольного выражения от исходных столбцов. В этом случае для получения доступа к результату этого выражения рекомендуется присваивать ему имя с помощью `AS`, см. второй [пример](group-by.md#examples).
...
В отличие от обычного GROUP BY, отключается стадия Map-side combiner и дополнительные Reduce для каждого поля с [DISTINCT](group-by.md#distinct) агрегацией.
...
`HOP` группирует данные по перекрывающимся временным окнам ([hopping windows](<https://en.wikipedia.org/wiki/Window_function_(SQL)#Hopping_window>)). Поддерживается как в [аналитических запросах к таблицам](group-by.md#hop-table), так и в [потоковых запросах к топикам](group-by.md#hop-topic).
...
Параметр `delay` при аналитической обработке таблицы **не используется**: данные уже доступны целиком, порядок обхода задаётся сортировкой по `time_extractor`, а завершение окна определяется алгоритмом полного прохода по группе (см. правило закрытия в [описании](group-by.md#hop-description) выше).
...
Алгоритм построения окон совпадает с [GROUP BY HOP](group-by.md#group-by-hop): окна начинаются в моменты, кратные `hop`, и имеют длительность `interval`. Событие попадает во все окна, временной диапазон которых включает его время.

### Аналитический `HoppingWindow`поверх таблицы {#hopping-window-table}

При работе с таблицами `HoppingWindow` выполняет группировку по временным окнам аналогично [HOP](group-by.md#group-by-hop), но без параметра `delay`, который при аналитическом использовании всегда игнорировался (данные в таблице уже отсортированы).
...
При работе с [топиками](../../../../concepts/datamodel/topic.md) `HoppingWindow` использует [watermarks](../../../../dev/streaming-query/watermarks.md) для определения момента закрытия окна. Окно закрывается, когда значение watermark не меньше конца окна. Это обеспечивает более точные результаты агрегации по сравнению с `HOP`, где окно закрывается по фиксированному `delay`.
...
Для корректной работы `HoppingWindow` в потоковом режиме необходимо настроить watermark в секции [WITH](with.md) источника. Подробнее: [Настройка](../../../../dev/streaming-query/watermarks.md#configuration).
