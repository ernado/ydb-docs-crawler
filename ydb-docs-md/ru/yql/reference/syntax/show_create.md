---
title: "SHOW CREATE"
url: "https://ydb.tech/docs/ru/yql/reference/syntax/show_create?version=v26.1"
doc_path: "ru/yql/reference/syntax/show_create"
version: "v26.1"
lang: "ru"
...
## Результат выполнения {#rezultat-vypolneniya}
...
- **CreateQuery** — полный набор DDL-выражений, необходимых для создания объекта:
  ...
  - Для таблиц: основной оператор [CREATE TABLE](create_table/index.md) (с путем относительно базы), а также дополнительные команды, необходимые для описания текущего состояния и настроек:
    ...
    - [ALTER TABLE ... ALTER INDEX](alter_table/secondary_index.md#alter-index)— для задания настроек партицирования вторичных индексов.
    - [ALTER TABLE ... ADD CHANGEFEED](alter_table/changefeed.md)— для добавления потока изменений.
    - `ALTER SEQUENCE` — для восстановления состояния `Sequence` у колонок типа [Serial](../types/serial.md).
...
### Строковые таблицы {#strokovye-tablicy}
...
### Колоночные таблицы {#kolonochnye-tablicy}
