---
title: "Выражения"
url: "https://ydb.tech/docs/ru/yql/reference/syntax/expressions?version=v26.1"
doc_path: "ru/yql/reference/syntax/expressions"
version: "v26.1"
lang: "ru"
source_path: "ru/core/yql/reference/syntax/expressions.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/yql/reference/syntax/expressions.md"
description: "Конкатенация строк. Выполняется через бинарный оператор ||."
...
| 3 | `a\|\|b` | [Конкатенация строк](expressions.md#concatenation) | Левая |
...
Проверка на пустое значение (`NULL`). Так как `NULL` является особым значением, которое [ничему не равно](../types/optional.md#null_expr), то обычные [операторы сравнения](expressions.md#comparison-operators) для этой задачи не подходят.
...
Сравнение двух значений. В отличие от обычных [операторов сравнения](expressions.md#comparison-operators), нуллы считаются равными друг другу.
...
> [!WARNING]
> В отличие от аналогичного ключевого слова в Python, в YQL `IN` **НЕ** является поиском подстроки в строке. Для поиска подстроки можно использовать функцию [String::Contains](../udf/list/string.md) или описанные выше [LIKE / REGEXP](expressions.md#check-match).
...
- При явном приведении типов данных для указания целевого типа, см. [CAST](expressions.md#cast).
...
У каждого именованного выражения есть область видимости. Она начинается сразу после определения именованного выражения и заканчивается в конце ближайшего охватывающего scope имен (например в конце запроса либо в конце тела [лямбда-функции](expressions.md#lambda), [ACTION](action.md#define-action)).  
...
Анонимные имена аргументов поддерживаются также для [лямбда-функций](expressions.md#lambda), [ACTION](action.md#define-action).
...
- [именованные подзапросы](expressions.md#named-nodes): `$foo = SELECT * FROM T;` (использование именованного подзапроса `$foo` является табличным выражением)
...
- табличный контекст - после [FROM](select/from.md).
  ...
  Табличный контекст также возникает после [UNION ALL](select/index.md#unionall), [JOIN](select/join.md#join);

- векторный контекст - после [IN](expressions.md#in). В этом контексте табличное выражение обязано содержать ровно одну колонку (имя этой колонки никак не влияет на результат выражения).
...
В круглых скобках перечисляются аргументы, далее после стрелки указывается тело lambda. Тело lambda состоит либо из выражения в круглых скобках, либо из фигурных скобок вокруг необязательной цепочки выражений с присвоением [именованных выражений](expressions.md#named-nodes) и результата вызова после ключевого слова `RETURN` в последнем выражении.
