---
title: "ru/yql/reference/syntax/select"
url: "https://ydb.tech/docs/ru/yql/reference/syntax/select?version=v26.1"
doc_path: "ru/yql/reference/syntax/select"
version: "v26.1"
lang: "ru"
source_path: "ru/core/yql/reference/syntax/select/index.md"
vcs_url: "https://github.com/ydb-platform/ydb/tree/main/ydb/docs/ru/core/yql/reference/syntax/select/index.md"
description: "SELECT. Возвращает результат вычисления выражений, указанных после SELECT. Может использоваться в сочетании с другими операциями для получения иного эффекта."
revision: "e9f541853a7760e5c0d0babc071d86df7f523cf5"
---

# ru/yql/reference/syntax/select
...
- выполняется [FLATTEN COLUMNS](select/flatten.md#flatten-columns) или [FLATTEN BY](select/flatten.md); алиасы, заданные во `FLATTEN BY`, становятся видны после этой точки;
- выполняются все [JOIN](select/join.md);
- к полученным данным добавляются (или заменяются) колонки, заданные в [GROUP BY ... AS ...](select/group-by.md);
- выполняется [WHERE](select/where.md): все данные, не удовлетворяющие предикату, отфильтровываются;
- выполняется [GROUP BY](select/group-by.md), вычисляются значения агрегатных функций;
- выполняется фильтрация [HAVING](select/group-by.md#having);
- вычисляются значения [оконных функций](select/window.md);
- вычисляются выражения в `SELECT`;
- выражениям в `SELECT` назначаются имена заданные алиасами;
- к полученным таким образом колонкам применяется top-level [DISTINCT](select/distinct.md);
- таким же образом вычисляются все подзапросы в [UNION ALL](select/union.md#union-all), выполняется их объединение (см. [PRAGMA AnsiOrderByLimitInUnionAll](pragma.md#pragmas));
- выполняется сортировка согласно [ORDER BY](select/order_by.md);
- к полученному результату применяются [OFFSET и LIMIT](select/limit_offset.md).
...
В стандартном SQL порядок колонок указанных в проекции (в `SELECT`) имеет значение. Помимо того, что порядок колонок должен сохраняться при отображении результатов запроса или при записи в новую таблицу, некоторые конструкции SQL этот порядок используют.  
 Это относится в том числе к [UNION ALL](select/union.md#union-all) и к позиционному [ORDER BY](select/order_by.md) (ORDER BY ordinal).
...
- порядок колонок после [JOIN](select/join.md): сначала колонки левой стороны, потом правой. Если порядок какой-либо из сторон, присутствующей в выходе `JOIN`, не определен, порядок колонок результата также не определен;
- порядок `UNION ALL` зависит от режима выполнения [UNION ALL](select/union.md#union-all);
- порядок колонок для [AS_TABLE](select/from_as_table.md) не определен;
...
## Обращение к нескольким таблицам в одном запросе {#obrashenie-k-neskolkim-tablicam-v-odnom-zaprose}

В стандартном SQL для выполнения запроса по нескольким таблицам используется [UNION ALL](select/union.md#union-all), который объединяет результаты двух и более `SELECT`. Это не совсем удобно для сценария использования, в котором требуется выполнить один и тот же запрос по нескольким таблицам (например, содержащим данные на разные даты). В YQL, чтобы было удобнее, в `SELECT` после `FROM` можно указывать не только одну таблицу или подзапрос, но и вызывать встроенные функции, позволяющие объединять данные нескольких таблиц.
...
## Поддерживаемые конструкции в SELECT {#podderzhivaemye-konstrukcii-v-select}
