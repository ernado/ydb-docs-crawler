---
title: "DateTime"
url: "https://ydb.tech/docs/ru/yql/reference/udf/list/datetime?version=v26.1"
doc_path: "ru/yql/reference/udf/list/datetime"
version: "v26.1"
lang: "ru"
...
Модуль DateTime предоставляет набор функций для работы с датами и временем в YDB. Они поддерживают все доступные типы данных временных меток и интервалов как с [базовым диапазоном допустимых значений](../../types/primitive.md#datetime-basic), так и с [расширенным](../../types/primitive.md#datetime-extended). Кроме того, эти функции используют собственное внутреннее представление значений, в котором каждое значение разбивается на отдельные компоненты. Это представление описано [ниже](datetime.md#resource).
...
Большинство функций модуля DateTime оперируют не исходными временными метками и интервалами, а внутренним представлением, основанным на специальном типе данных [ресурс](../../types/special.md). Оно представляет собой указатель на структуру данных в памяти, где компоненты времени хранятся раздельно. Работа с ресурсом происходит исключительно через функции модуля DateTime. Типовой сценарий работы с этим модулем (примеры см. [в конце статьи](datetime.md#tipovye-scenarii)) следующий:

1. Вызвать функцию [`Split`](datetime.md#split), чтобы получить из исходной временной метки внутреннее представление, либо [`Parse...`](datetime.md#parse), чтобы получить его из строки.

2. При необходимости вызвать функции, модифицирующие это внутреннее представление нужным образом, например [`Shift...`](datetime.md#shift), [`StartOf...`](datetime.md#startof) или [`Update...`](datetime.md#update).
...
Преобразование из простого типа YDB во [внутреннее представление](datetime.md#resource) модуля DateTime. Преобразование всегда выполняется успешно, при условии что на вход функции подаётся требуемое значение.
...
Сборка простого типа YDB из [внутреннего представления](datetime.md#resource) модуля DateTime. Преобразование всегда выполняется успешно, если на вход функции подаётся требуемое значение.

### Список функций {#spisok-funkcij1}
...
Получение компоненты [внутреннего представления](datetime.md#resource).

### Список функций {#spisok-funkcij2}
...
Обновление одной или нескольких компонент во [внутреннем представлении](datetime.md#resource). Возвращает либо обновлённую копию, либо `NULL`, если после обновления получается некорректная дата или возникают другие противоречия.

### Список функций {#spisok-funkcij3}
...
### Список функций {#spisok-funkcij4}
...
### Список функций {#spisok-funkcij5}
...
### Список функций {#spisok-funkcij6}
...
### Список функций {#spisok-funkcij7}
...
### Список функций {#spisok-funkcij8}
...
### Список функций {#spisok-funkcij9}
...
### Список функций {#spisok-funkcij10}
...
## Parse конкретных форматов {#parse-konkretnyh-formatov}
...
### Список функций {#spisok-funkcij11}
...
### Преобразования между строками и секундами {#preobrazovaniya-mezhdu-strokami-i-sekundami}
...
### Форматирование даты и времени {#formatirovanie-daty-i-vremeni}
...
### Преобразование типов {#preobrazovanie-tipov}
...
### Преобразование времени в дату {#preobrazovanie-vremeni-v-datu}
