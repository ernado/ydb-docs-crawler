---
title: "Функции для работы со структурами"
url: "https://ydb.tech/docs/ru/yql/reference/builtins/struct?version=v26.1"
doc_path: "ru/yql/reference/builtins/struct"
version: "v26.1"
lang: "ru"
...
Добавление одного нового поля в структуру. Если необходимо добавление нескольких полей, предпочтительнее использовать [ExpandStruct](struct.md#expandstruct).
...
Если указанного поля не существовало, в отличии от [RemoveMember](struct.md#removemember) ошибка возвращена не будет.
...
3. `mergeLambda` - *(опционально)* позволяет задать функцию для объединения полей (аргументы: имя поля, `Optional` значение поля в первой структуре, `Optional` значение поля во второй структуре - аргументы принимают значение `Nothing<T?>` в случае отсутствия соответствующего поля); по умолчанию выбирается значение поля из первой структуры, а если в первой отсутствует — из второй.
